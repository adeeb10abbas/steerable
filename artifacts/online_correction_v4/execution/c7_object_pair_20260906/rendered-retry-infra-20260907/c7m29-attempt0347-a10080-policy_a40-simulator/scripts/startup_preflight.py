#!/usr/bin/env python3
"""Fail-closed startup qualification for a V4 Kubernetes lane.
...
SCHEMA_VERSION = "vla-wam-v4-k8s-lane-startup-preflight-v1"
...
def reserve_policy_port(port: int):
    ...
    path = Path(os.environ.get("XDG_RUNTIME_DIR", "/tmp")) / f"v4-lane-policy-port-{port}.lock"
    try:
...
def verify_writable_parent(output_parent: Path) -> dict[str, Any]:
    ...
    try:
        fd, name = tempfile.mkstemp(prefix=".v4-lane-write-probe-", dir=output_parent)
        os.write(fd, b"v4-lane\n")
        os.fsync(fd)
        os.close(fd)
        Path(name).unlink()
    ...
    *,
    ...
    try:
        ...
        _write_json_exclusive(
            startup_path,
            {
                "schema_version": "vla-wam-v4-k8s-lane-runtime-startup-v1",
                "created_at_utc": utc_now(),
                "pod_identity": dict(pod_identity),
                "role": role,
                "lane_id": lane_id,
