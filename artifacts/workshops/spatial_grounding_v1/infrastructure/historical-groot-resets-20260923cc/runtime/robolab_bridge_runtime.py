...
class StateCaptureProxy:
    ...
    def reset(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if final_path.exists() or (self._reset_count == 0 and self._stream_path.exists()):
        ...
        # RoboLab's frozen evaluator intentionally calls reset twice before
        # inference.  Preserve the first (warm-up) reset in a separate raw
        # sidecar, then begin the behavioral stream at the second identical
        # seeded reset.  A reset after any executed action remains invalid.
        if self._reset_count:
            if len(self._samples) != 1:
                raise RuntimeError(
                    "refusing to reset GR00T state capture after actions were executed"
                )
            warmup_path = self._capture_dir / (
                f"seed{args_cli.environment_seed}_{self._relation}_"
                f"warmup_reset{self._reset_count - 1}.jsonl"
            )
            if warmup_path.exists():
                raise FileExistsError(
                    f"refusing to overwrite retained warm-up reset: {warmup_path}"
                )
            self._stream_path.replace(warmup_path)
        self._samples = []
        self._started = time.monotonic()
        self._capture_dir.mkdir(parents=True, exist_ok=True)
