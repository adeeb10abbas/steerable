...
class StateCaptureProxy:
    ...
    def __init__(self, env: Any, task_name: str, capture_dir: Path) -> None:
        ...
        self._written = False
        self._reset_count = 0
        self._canonical_partial_path = capture_dir / (
            f"seed{args_cli.environment_seed}_{self._relation}_states.partial.jsonl"
        )
        self._partial_path = self._canonical_partial_path
        observed_cap = int(env.max_episode_length)
        if observed_cap != 450:
            raise ValueError(
                f"frozen pi0.5 environment must expose 450 steps, got {observed_cap}"
            )
        capture_dir.mkdir(parents=True, exist_ok=True)
        diagnostic = {
            "schema_version": "vla-wam-shared-v3-pi05-runtime-diagnostic-v1",
            "environment_seed": args_cli.environment_seed,
            "requested_relation": self._relation,
            "underlying_max_episode_length": observed_cap,
            "underlying_max_episode_length_s": float(env.max_episode_length_s),
            "underlying_step_dt": float(env.step_dt),
        }
        with (capture_dir / "runtime_diagnostics.jsonl").open("a", encoding="utf-8") as handle:
    ...
    def max_episode_length(self) -> int:
        """Expose and enforce the frozen 30 s / 15 Hz action cap."""

        observed = int(self._env.max_episode_length)
        if observed != 450:
            raise ValueError(f"pi0.5 action-cap drift: {observed} != 450")
        return observed
    ...
    def reset(self, *args: Any, **kwargs: Any) -> Any:
        ...
        if final_path.exists() or self._reset_count >= 2 or len(self._samples) > 1:
            raise FileExistsError(
                "refusing an unexpected reset after pi0.5 behavioral execution: "
                f"reset_count={self._reset_count}, samples={len(self._samples)}, "
                f"final={final_path}"
            )
        # RoboLab's frozen episode runner performs two resets before action 0.
        # Preserve the setup reset separately, then use the canonical partial
        # stream for the second (behavioral) reset without overwriting either.
        self._partial_path = (
            self._capture_dir
            / f"seed{args_cli.environment_seed}_{self._relation}_setup_reset0.partial.jsonl"
            if self._reset_count == 0
            else self._canonical_partial_path
        )
        if self._partial_path.exists():
            raise FileExistsError(
                f"refusing to overwrite retained pi0.5 reset evidence: {self._partial_path}"
            )
