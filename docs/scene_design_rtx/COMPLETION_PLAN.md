...
1. **DIST geometry:** new `scene_completion_dist.py` and focused geometry tests; reuse a verified existing DIST arrangement with clean appearance. Test neutral relation, both goal signs, anchor/support separation and both bowl arrangements.
...
Two RTX 3090s already host this task's LAT and HEIGHT qualification workers. New scene code uses a separate module/snapshot so it cannot change their inputs. New work may be interleaved after candidate boundaries. Stop only for concrete infrastructure failure, lack of space, or an exhausted declared pool; diagnose from retained evidence and register a new geometry revision when necessary. Do not rerun valid failures to improve a denominator.
