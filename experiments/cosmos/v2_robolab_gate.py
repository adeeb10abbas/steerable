#!/usr/bin/env python3
"""Run exactly one frozen Cosmos3 LEFT/RIGHT DROID seed pair in RoboLab."""
...
parser.add_argument("--remote-port", type=int, default=18010)
...
if args_cli.open_loop_horizon != 32:
    parser.error("The frozen Cosmos DROID gate requires --open-loop-horizon 32")
if args_cli.video_mode != "viewport":
    parser.error("The frozen Cosmos DROID gate requires --video-mode viewport")
...
auto_register_droid_envs(
    task=[str(left_task), str(right_task)], cameras=WRIST_LEFT_RIGHT_HEAD
)
args_cli.task = [
...
def make_client(_: argparse.Namespace) -> V2Cosmos3Client:
    return V2Cosmos3Client(
...
def main() -> None:
    run_evaluation(args_cli, policy="cosmos3_v2", client_factory=make_client)
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Cosmos3 v2] technical failure: {exc}")
        traceback.print_exc()
        simulation_app.close()
        raise
