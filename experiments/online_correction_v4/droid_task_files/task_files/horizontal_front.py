"""V4 horizontal front relation: timeout-only rubiks_cube_banana_bowl task."""
...
@dataclass
class V4HorizontalFrontTask(Task):
    contact_object_list = list(shared.CONTACT_OBJECT_LIST)
    scene = shared.scene_for_env_seed(
        shared.bound_instruction_for_relation("front").env_seed
    )
    terminations = shared.timeout_only_termination()
    instruction = shared.instruction_dict_for_relation("front")
    attributes = shared.task_attributes("front")
