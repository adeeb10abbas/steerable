#!/usr/bin/env python3
"""Plan or run the released V3-B005 Nano queue without overwriting evidence."""
...
def ordered_cells(release: ReleaseBundle) -> list[AuthorizedCell]:
    ...
    if len(cells) != 210:
        raise RuntimeContractError("live queue must contain exactly 210 cells")
    return cells
...
def _attempt_dir(raw_root: Path, cell: AuthorizedCell) -> Path:
    return (
        ...
        / "V3-B005_cosmos3_nano_lateral_sweep"
...
def main() -> None:
    ...
    verify_behavioral_release_gate(args.release_gate, release=release, runtime=runtime)
    if not args.safe_fixture.is_file() or sha256_file(args.safe_fixture) != release.safe_fixture_sha256:
        raise RuntimeContractError("safe fixture path does not match released SHA-256")
