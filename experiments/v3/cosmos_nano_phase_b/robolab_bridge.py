#!/usr/bin/env python3
"""Run one exact released V3-B001 Nano cell in RoboLab.
...
from experiments.v3.cosmos_nano_phase_b.live_support import (  # noqa: E402
...
candidate_record = release.amendment.get("calibration_evidence", {}).get("candidate", {})
candidate_path = bootstrap.fixture_candidate.resolve()
if (
    not candidate_path.is_file()
    or sha256_file(candidate_path) != bootstrap.fixture_candidate_sha256
    or candidate_record.get("sha256") != bootstrap.fixture_candidate_sha256
):
    BOOTSTRAP.error("fixture candidate does not match the released calibration evidence")
...
from experiments.v3.cosmos_nano_phase_b.live_client import V3BNanoLiveClient  # noqa: E402
...
TASK_FILES = {
    ("control", "left"): "control_left.py",
    ("control", "right"): "control_right.py",
    ("position_mirrored", "left"): "position_mirrored_left.py",
    ("position_mirrored", "right"): "position_mirrored_right.py",
}
TASK_NAMES = {
    ("control", "left"): "V3BNanoControlLeftCalibrationTask",
    ("control", "right"): "V3BNanoControlRightCalibrationTask",
...
class StateCaptureProxy:
    ...
    def write_reset_attestation(self) -> Path:
        ...
        for directory in (bootstrap.reset_attestation.parent, args_cli.output_dir):
            ...
            probe.write_bytes(b"v3b001-live-output-preflight\n")
            probe.unlink()
        reset = {
...
def _write_export() -> Path:
    ...
    export = {
        ...
        "source_artifacts": {
            ...
            "fixture_candidate": str(candidate_path),
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Nano V3-B001] infrastructure failure: {error}")
        traceback.print_exc()
        raise
