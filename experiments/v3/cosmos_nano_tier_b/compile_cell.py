#!/usr/bin/env python3
"""Compile one released V3-B008/B009 Nano cell into retained raw episode JSONL.
...
from experiments.v3.cosmos_nano_tier_b.live_client import validate_reset_attestation
...
def _validate_steps(export: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for index, step in enumerate(raw):
        ...
        row: dict[str, Any] = {
            ...
            "object_grabbed": step.get("object_grabbed"),
        }
        _in_cone(row, "left")
        if type(row["grippers_open"]) is not bool:
            _fail("grippers_open must be a retained raw boolean")
        if type(row["object_grabbed"]) is not bool:
            _fail("object_grabbed must be a retained raw boolean")
...
def build_behavioral_record(
    *,
    ...
    export = load_json(export_path, "Tier-B simulator export")
    ...
    grasp_step = next((index for index, step in enumerate(steps) if step["object_grabbed"]), None)
    diagnostics = {
        "schema_version": "vla-wam-shared-v3b008-v3b009-nano-episode-diagnostics-v1",
        "success": requested_success,
        "failure_category": normalized["failure_taxonomy"],
        "signed_final_lateral_offset_m": normalized_measurements["signed_final_lateral_offset_m"],
        "requested_side_depth_m": normalized_measurements["final_requested_signed_margin_m"],
        "cone_entry_step": normalized_measurements["first_requested_entry_step"],
        "cone_entry_sustained": normalized_measurements["first_sustained_requested_entry_step"] is not None,
        "episode_length_steps": actions_executed,
        "time_to_first_contact_steps": normalized_measurements["first_contact_step"],
        "first_contact_status": normalized_measurements["first_contact_status"],
        "first_contact_unavailable_reason": normalized_measurements["first_contact_unavailable_reason"],
    ...
    for key, value in diagnostics.items():
        if key not in {"schema_version", "endpoint_shift_m", "action_distinct", "pair_diagnostics_location"}:
            normalized[key] = value
    normalized["nano_tier_b_diagnostics"] = diagnostics
    return normalized
...
def compile_cell(
    *,
    ...
    release = load_release(study_root, amendment_id, release_manifest)
    if release.manifest_sha256 != release_manifest_sha256:
        _fail("release manifest CLI hash mismatch")
    cell = release.cell(cell_id)
    runtime = load_runtime(runtime_manifest, study_root=study_root, release=release)
    validate_behavioral_release_gate(release_gate, release=release, runtime=runtime)
