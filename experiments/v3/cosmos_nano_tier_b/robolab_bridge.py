#!/usr/bin/env python3
"""Run one exact released V3-B008/B009 Nano cell in RoboLab.
...
BOOTSTRAP.add_argument("--remote-port", type=int, default=18011)
...
from experiments.v3.cosmos_nano_tier_b.live_client import validate_reset_attestation  # noqa: E402
...
RESET_SCHEMA = "vla-wam-shared-v3b008-v3b009-nano-reset-v1"
SETTLE_EVIDENCE_SCHEMA = "vla-wam-shared-v3b008-v3b009-nano-settle-v1"
...
if bootstrap.release_manifest_sha256 != release.manifest_sha256:
    BOOTSTRAP.error("release manifest CLI hash mismatch")
cell = release.cell(bootstrap.cell_id)
runtime_identity = load_runtime(bootstrap.runtime_manifest, study_root=study_root, release=release)
release_gate = validate_behavioral_release_gate(
    bootstrap.release_gate, release=release, runtime=runtime_identity
)
...
if (
    ...
    or cell.row["candidate_sha256"] != bootstrap.safe_fixture_sha256
):
    BOOTSTRAP.error("model-blind candidate does not match the released cell")
os.environ["VLA_WAM_V3_TIERB_GATE_CANDIDATE"] = str(safe_fixture_path)
os.environ["VLA_WAM_V3_TIERB_GATE_SHA256"] = bootstrap.safe_fixture_sha256
...
def _expected_positions_robot_m() -> dict[str, list[float]]:
    return {
        name: list(position)
        for name, position in cell.row["fixture_positions_robot_base_m"].items()
    }


def validate_settle_stability_evidence(
    evidence: dict[str, Any], *, cell: Any, runner_reset_contract_complete: bool = True
) -> None:
    expected = {
    ...
    for key, wanted in expected.items():
        if evidence.get(key) != wanted:
            raise RuntimeError(f"settle evidence mismatch for {key}")
    if runner_reset_contract_complete and (
        evidence.get("runner_pre_action_reset_calls") != 2
        or evidence.get("duplicate_second_reset_idempotent") is not True
    ):
        raise RuntimeError("RoboLab duplicate-reset contract did not complete")
    maxima = evidence.get("stability_window_component_maxima", {})
    if set(maxima) != set(SETTLE_OBJECTS):
        raise RuntimeError("settle evidence object set mismatch")
    for name in SETTLE_OBJECTS:
        if maxima[name]["max_linear_component_speed_m_s"] > LINEAR_SPEED_TOLERANCE_M_S:
            raise RuntimeError(f"{name} remained linearly unstable")
        if maxima[name]["max_angular_component_speed_rad_s"] > ANGULAR_SPEED_TOLERANCE_RAD_S:
            raise RuntimeError(f"{name} remained angularly unstable")
...
for output in (bootstrap.reset_attestation, bootstrap.simulator_export, bridge_failure_path):
    if output.exists():
        BOOTSTRAP.error(f"refusing to overwrite retained Tier-B evidence: {output}")
for directory in (
    ...
    if directory.exists():
        BOOTSTRAP.error(f"refusing to reuse retained Tier-B output directory: {directory}")
...
if args_cli.video_mode != "viewport":
    parser.error("every released Tier-B cell requires viewport video")
if args_cli.num_envs != 1 or args_cli.num_runs != 1:
    parser.error("a released Tier-B cell requires exactly one environment and one run")
...
if args_cli.instruction_type != "default":
    parser.error("Tier-B uses only the exact static direct-command prompt")
...
from experiments.v3.cosmos_nano_tier_b.live_client import NanoTierBLiveClient  # noqa: E402
...
TASKS = {
    ("V3-B008", "target_start_left", "left"): (
        "v3b008_target_start_left_left.py", "V3B008TargetStartLeftLeftGateTask"
    ),
    ("V3-B008", "target_start_left", "right"): (
        "v3b008_target_start_left_right.py", "V3B008TargetStartLeftRightGateTask"
    ),
    ("V3-B008", "target_start_center", "left"): (
        "v3b008_target_start_center_left.py", "V3B008TargetStartCenterLeftGateTask"
    ),
    ("V3-B008", "target_start_center", "right"): (
        "v3b008_target_start_center_right.py", "V3B008TargetStartCenterRightGateTask"
    ),
    ("V3-B008", "target_start_right", "left"): (
        "v3b008_target_start_right_left.py", "V3B008TargetStartRightLeftGateTask"
    ),
    ("V3-B008", "target_start_right", "right"): (
        "v3b008_target_start_right_right.py", "V3B008TargetStartRightRightGateTask"
    ),
    ("V3-B009", "cube_target_bowl_reference", "left"): (
        "v3b009_cube_target_bowl_reference_left.py", "V3B009CubeTargetBowlReferenceLeftGateTask"
    ),
    ("V3-B009", "cube_target_bowl_reference", "right"): (
        "v3b009_cube_target_bowl_reference_right.py", "V3B009CubeTargetBowlReferenceRightGateTask"
    ),
    ("V3-B009", "bowl_target_cube_reference", "left"): (
        "v3b009_bowl_target_cube_reference_left.py", "V3B009BowlTargetCubeReferenceLeftGateTask"
    ),
    ("V3-B009", "bowl_target_cube_reference", "right"): (
        "v3b009_bowl_target_cube_reference_right.py", "V3B009BowlTargetCubeReferenceRightGateTask"
    ),
}
task_root = study_root / "experiments/v3/prospective_tier_b_gates/task_files"
task_file, task_name = TASKS[(release.amendment_id, cell.arm, cell.relation)]
auto_register_droid_envs(
    task=[str(task_root / task_file)],
...
MATCHED_BLOCK_ID = f"{release.amendment_id.lower().replace('-', '')}:nano:seed{cell.seed}"
...
class StateCaptureProxy:
    ...
    def __init__(self, env: Any, capture_dir: Path) -> None:
        ...
        for path in (
            ...
            if path.exists():
                raise FileExistsError(f"refusing to overwrite Tier-B state evidence: {path}")

    def __getattr__(self, name: str) -> Any:
        return getattr(self._env, name)

    def _sample(self, action_step: int) -> dict[str, Any]:
        object_world = self._env.scene[TARGET_OBJECT].data.root_pos_w[0].detach().cpu().numpy()
        reference_world = self._env.scene[REFERENCE_OBJECT].data.root_pos_w[0].detach().cpu().numpy()
        ...
        return {
            ...
            "object_xyz": _quat_inverse_rotate_wxyz(robot_quat, object_world - robot_pos).tolist(),
            "reference_xyz": _quat_inverse_rotate_wxyz(robot_quat, reference_world - robot_pos).tolist(),
            "grippers_open": bool(
                object_dropped(self._env, object=TARGET_OBJECT, env_id=0)
            ),
            "object_grabbed": bool(
                object_grabbed(self._env, object=TARGET_OBJECT, env_id=0)
            ),
        }
    ...
    def write_reset_attestation(self) -> Path:
        ...
        evidence = {
            ...
            "registered_cell_id": cell.cell_id,
            "candidate_sha256": cell.row["candidate_sha256"],
            "arm": cell.arm,
        ...
        for directory in (bootstrap.reset_attestation.parent, args_cli.output_dir):
            ...
            probe.write_bytes(b"v3-tier-b-live-output-preflight\n")
            probe.unlink()
        reset = {
            ...
            "prompt_sha256": sha256_bytes(cell.row["prompt"].encode("utf-8")),
            ...
            "candidate_sha256": bootstrap.safe_fixture_sha256,
            "live_bridge_source_sha256": sha256_file(Path(__file__)),
            "live_client_source_sha256": sha256_file(
                study_root / "experiments/v3/cosmos_nano_tier_b/live_client.py"
            ),
        }
    ...
    def reset(self, *args: Any, **kwargs: Any) -> Any:
        if self._partial_started or len(self._samples) > 1 or bootstrap.reset_attestation.exists():
            raise RuntimeError("RoboLab attempted a reset after Tier-B request/action authorization")
        ...
        left = bool(object_left_of(
            self._env, object=TARGET_OBJECT, reference_object=REFERENCE_OBJECT,
    ...
    def write_capture(self) -> Path | None:
        ...
        capture = {
            ...
            "registered_cell_id": cell.cell_id,
            "attempt_id": f"{args_cli.output_folder_name}:{cell.arm}:{cell.relation}",
            "environment_seed": cell.seed,
            "sampling_seed": cell.seed,
...
clients: list[NanoTierBLiveClient] = []
...
def make_client(_: argparse.Namespace) -> NanoTierBLiveClient:
    client = NanoTierBLiveClient(
...
def _write_export() -> Path:
    ...
    if capture.get("behavioral_result_valid_candidate") is not True:
        raise RuntimeError("partial Tier-B attempt cannot emit a behavioral simulator export")
    ...
    export = {
        ...
        "source_artifacts": {
            ...
            "model_blind_candidate": str(safe_fixture_path),
...
if __name__ == "__main__":
    try:
        ...
        print(f"[Nano V3-B008/B009] infrastructure failure: {error}")
        traceback.print_exc()
        raise
