#!/usr/bin/env python3
"""Execute one released eight-cell Cosmos V3-C001 seed block.
...
from experiments.v3.cosmos_droid.contract import (  # noqa: E402
    MODEL_CONTRACTS as PHASE_A_MODEL_CONTRACTS,
...
from experiments.v3.phase_c_four_phrasings.cosmos_behavioral_contract import (  # noqa: E402
    prompt_condition,
...
if bootstrap.open_loop_horizon != 32:
    BOOTSTRAP.error("the frozen Cosmos open-loop horizon is 32")
if bootstrap.remote_port != PHASE_A_MODEL_CONTRACTS[MODEL_ID]["server_port"]:
    BOOTSTRAP.error("remote port differs from the model-specific Cosmos serving contract")
...
runtime_identity = verify_runtime_identity(study_root, MODEL_ID, runtime_identity_path)
if runtime_identity["runtime_identity_sha256"] != released.runtime_identity_sha256:
    BOOTSTRAP.error("live Cosmos runtime identity differs from the Phase-C release")
if validate_task_sources(study_root) != bridge.get("task_source_sha256"):
    BOOTSTRAP.error("Cosmos Phase-C task sources changed after preflight")
...
if bootstrap.runner_output_root.exists() or bootstrap.launch_evidence.exists():
    BOOTSTRAP.error("refusing to overwrite retained Cosmos Phase-C launch output")
...
if args_cli.video_mode != "viewport":
    parser.error("V3-C001 Cosmos requires viewport video")
if args_cli.num_envs != 1 or args_cli.num_runs != 1:
    parser.error("V3-C001 Cosmos requires one environment and one run per cell")
...
if args_cli.renderer != "realtime" or args_cli.rendering_type != "balanced":
    parser.error("V3-C001 Cosmos requires the released realtime balanced renderer")
...
sys.path.insert(0, str(study_root / "experiments/cosmos"))
...
if len(cell_by_task) != 8:
    raise ValueError("the Cosmos Phase-C task map is not one-to-one")
auto_register_droid_envs(
...
class StateCaptureProxy:
    ...
    def reset(self, *args: Any, **kwargs: Any) -> Any:
        if self.actions_started:
            raise RuntimeError("unexpected environment reset after Cosmos actions began")
    ...
    def step(self, action: Any) -> Any:
        if not self.samples:
            raise RuntimeError("Cosmos action attempted before the matched reset")
        if not self.actions_started:
            raw_dir = Path(self.cell["raw_cell_directory"])
            # The Cosmos client retains the first returned action chunk and
            # decoded future before RoboLab executes its first action, so it
            # legitimately creates this fresh cell directory first.
    ...
    def write_capture(self) -> None:
        ...
        capture = {
            "schema_version": "vla-wam-shared-v3c-cosmos-state-capture-v1",
            ...
            "first_contact_unavailable_reason": (
                "RoboLab frozen Cosmos adapter exposes grasp and detached-release "
        ...
        if self.capture_path.exists():
            raise FileExistsError(f"refusing to overwrite Cosmos Phase-C capture: {self.capture_path}")
...
class PhaseCCosmosClient(V2Cosmos3Client):
    """Exact-prompt client retaining every action and decoded future."""

    def __init__(self, *, cell: dict[str, Any], **kwargs: Any) -> None:
        self.cell = cell
        self.outputs = validate_live_output_contract(cell, fresh=False)
        super().__init__(
            action_trace_dir=Path(cell["raw_cell_directory"]),
            future_trace_dir=self.outputs["decoded_future_directory"],
            **kwargs,
        )

    def _relation(self) -> str:
        # Contrastive prompts contain both direction words, so the inherited
        # substring parser is not a valid router for V3-C001.
        return self.cell["relation"]

    def infer(self, obs: Any, instruction: str, *, env_id: int = 0) -> dict:
        if instruction != self.cell["prompt"]:
            raise ValueError("live Cosmos instruction does not match exact registered bytes")
        if prompt_condition(instruction) != (self.cell["prompt_family"], self.cell["relation"]):
            raise ValueError("live Cosmos prompt routed to the wrong registered condition")
    ...
    def _unpack_response(self, response: dict[str, Any]) -> np.ndarray:
        ...
        self.request_records[-1].update(
            ...
            prompt_sha256=self.cell["prompt_sha256"],
            prompt_family=self.cell["prompt_family"],
            requested_relation=self.cell["relation"],
            environment_seed=self.cell["environment_seed"],
    ...
    def _write_trace(self) -> None:
        ...
        if self.prompt != self.cell["prompt"]:
            raise ValueError("Cosmos action trace prompt changed within the episode")
        source_actions = list(Path(self.cell["raw_cell_directory"]).glob(
            f"seed{bridge['seed']}_*_executed_actions.npy"
        ))
        source_metadata = list(Path(self.cell["raw_cell_directory"]).glob(
            f"seed{bridge['seed']}_*_executed_actions.json"
        ))
        if len(source_actions) != 1 or len(source_metadata) != 1:
            raise ValueError("Cosmos client did not emit one exact executed-action trace")
        if self.outputs["executed_actions"].exists():
            raise FileExistsError("refusing to overwrite Cosmos Phase-C actions")
        source_actions[0].replace(self.outputs["executed_actions"])
        actions = np.load(self.outputs["executed_actions"], allow_pickle=False)
        if actions.ndim != 2 or actions.shape[1] != 8:
            raise ValueError("Cosmos executed-action trace must be [steps,8]")
        metadata = json.loads(source_metadata[0].read_text())
        if len(metadata.get("requests", [])) != len(self.request_records):
            raise ValueError("Cosmos request metadata count changed during trace close")
        for record in self.request_records:
            ...
            if future.ndim != 4 or future.shape[0] != 33 or future.shape[-1] != 3:
                raise ValueError("Cosmos decoded future is not RGB [33,H,W,3]")
            if _sha256(future_path) != record["future_sha256"]:
                raise ValueError("Cosmos decoded future changed after retention")
        metadata.update({
            "schema_version": "vla-wam-shared-v3c-cosmos-action-future-trace-v1",
            "model_id": MODEL_ID,
            "registered_cell_id": self.cell["registered_cell_id"],
            "prompt": self.prompt,
            "prompt_sha256": self.cell["prompt_sha256"],
            ...
            "executed_action_shape": list(actions.shape),
            "requests": self.request_records,
            "model_request_count": len(self.request_records),
            "future_evidence_policy": (
                "Every exposed 33-frame decoded future is retained losslessly with a "
                "content hash; missing output is infrastructure-invalid, never zero."
            ),
        })
        self.outputs["action_future_metadata"].write_bytes(canonical_json_bytes(metadata))
        source_metadata[0].unlink()


_create_env = runtime.create_env
active_cell: dict[str, Any] | None = None
captures: dict[str, StateCaptureProxy] = {}
clients: dict[str, PhaseCCosmosClient] = {}
...
def _seeded_captured_create_env(*args: Any, **kwargs: Any) -> Any:
    ...
    if cell is None:
        raise ValueError(f"unregistered task reached Cosmos Phase-C bridge: {task_name}")
...
def make_client(_: argparse.Namespace) -> PhaseCCosmosClient:
    if active_cell is None:
        raise RuntimeError("Cosmos client constructed before its registered environment")
    client = PhaseCCosmosClient(
        cell=active_cell,
...
def _build_episode(cell: dict[str, Any], video: Path) -> dict[str, Any]:
    ...
    record = {
        ...
        "artifacts": {
            ...
            "decoded_future_trace": _file_record(outputs["action_future_metadata"]),
            "raw_result_jsonl": {
...
def _finalize() -> dict[str, Any]:
    ...
    if len(initial_hashes) != 1:
        raise ValueError("Cosmos Phase-C whole-seed cells did not share an identical reset")
    report = {
        ...
        "model_request_count": sum(len(client.request_records) for client in clients.values()),
...
def main() -> None:
    try:
        policy_id = "cosmos3_v2" if MODEL_ID == "cosmos3_edge_policy_droid" else "cosmos3_nano_v2"
