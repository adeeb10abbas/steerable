#!/usr/bin/env bash
# Run complete Cosmos V3-C001 seed blocks after the seed-8500 smoke passes.
#
...
if [[ "$model_id" != cosmos3_edge_policy_droid && "$model_id" != cosmos3_nano_policy_droid ]]; then
  echo "model-id must be cosmos3_edge_policy_droid or cosmos3_nano_policy_droid" >&2
  exit 2
fi
if [[ "$model_id" == cosmos3_edge_policy_droid && "$remote_port" != 18010 ]]; then
  echo "Edge Phase-C requires its frozen server port 18010" >&2
  exit 2
fi
if [[ "$model_id" == cosmos3_nano_policy_droid && "$remote_port" != 18011 ]]; then
  echo "Nano Phase-C requires its frozen server port 18011" >&2
  exit 2
fi
...
gate_root="$raw_root/phase_c_v3c001/$model_id"
behavioral_root="$raw_root/behavioral/v3-c001/$model_id"
attempt_tag=$(printf '%02d' "$attempt")
mkdir -p "$gate_root" "$behavioral_root"

# The released Cosmos servers are stateful.  Until an interleaved exact-repeat
# isolation gate exists, one queue owns one model server and executes every
# request serially.  A stale lock is intentionally fail-closed and must be
# preserved and diagnosed rather than silently removed.
single_client_lock="$gate_root/single_client_server.lock"
if ! mkdir "$single_client_lock"; then
  echo "refusing a second Cosmos client for $model_id: $single_client_lock" >&2
  exit 6
fi
cleanup_single_client_lock() {
  rmdir "$single_client_lock"
}
trap cleanup_single_client_lock EXIT
...
for seed in $(seq "$seed_start" "$seed_end"); do
  ...
  if [[ ! -s "$bridge_preflight" ]]; then
    PYTHONPATH="$study_root${PYTHONPATH:+:$PYTHONPATH}" /usr/bin/python3 -m \
      experiments.v3.phase_c_four_phrasings.cosmos_behavioral_bridge \
  ...
  run_guarded "$registration_attempt" \
    "$runtime_python" "$study_root/experiments/v3/phase_c_four_phrasings/cosmos_task_registration_preflight.py" \
      --study-root "$study_root" \
  ...
  run_guarded "$behavior_attempt" \
    "$runtime_python" "$study_root/experiments/v3/phase_c_four_phrasings/cosmos_live_bridge.py" \
      --study-root "$study_root" \
      --model-id "$model_id" \
      --bridge-preflight "$bridge_preflight" \
  ...
  echo "$model_id V3-C001 seed $seed complete: $launch_evidence"
done
