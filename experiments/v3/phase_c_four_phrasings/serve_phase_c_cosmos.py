"""Registered seed/prompt overlay for either Cosmos V3-C001 policy server.

The pinned official Edge and Nano servers keep their sampling seed in process
state.  Phase C instead registers one environment/policy seed per request.
This narrow overlay injects that registered seed into the unchanged official
inference method, echoes it for client verification, and serializes the full
request so two sessions cannot race through shared server state.
"""
...
_request_seed: ContextVar[int | None] = ContextVar("v3c001_cosmos_seed", default=None)
...
def _infer_registered_prompt(
    ...
    if type(seed) is not int or seed not in AUTHORIZED_SEEDS:
        raise ValueError("V3-C001 Cosmos sampling_seed must be one of 8500..8519")
    if not isinstance(prompt, str) or prompt not in AUTHORIZED_PROMPTS:
        raise ValueError("V3-C001 Cosmos prompt must match one exact registered string")
    with _request_lock:
