#!/usr/bin/env python3
"""Hash-bound π0.5 client for V3-E003 (15×8 action-only contract)."""
...
from experiments.v3.phase_e.bilateral_symmetry_null_control_v3e003.contract import (
    ACTION_CHUNK_STEPS,
    ACTION_DIM,
    AMENDMENT_ID,
    MODEL_ID,
...
class V3E003Pi05Client(Pi0DroidJointposClient):
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if sampling_seed_base not in SEEDS:
            raise ValueError("V3-E003 seeds are exactly 9400..9426")
        ...
        response = super()._query_server(
            {
                **request,
                ...
                "v3e003_release_fingerprint_sha256": self.release_fingerprint_sha256,
                "v3e003_reset_fingerprint_sha256": reset_hash,
            }
        )
    ...
    def trace_metadata(self) -> Mapping[str, Any]:
        ...
        stem = f"v3e003_seed{self.sampling_seed_base}"
        ...
        if any(path.exists() for path in (actions_path, chunks_path, metadata_path)):
            raise FileExistsError("refusing to overwrite retained V3-E003 action evidence")
