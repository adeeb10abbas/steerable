#!/usr/bin/env python3
"""Execute one hash-released π0.5 V3-E003 RoboLab cell.
...
from experiments.v3.phase_e.bilateral_symmetry_null_control_v3e003.contract import (  # noqa: E402
    ...
    STUDY_ID, canonical_json_bytes, sha256_bytes, sha256_file,
)
...
registration = json.loads(bootstrap.release_manifest.read_text(encoding="utf-8"))
if registration.get("schema_version") != "vla-wam-shared-v3e003-registration-v1" or registration.get("status") != "registered_before_inference":
    BOOTSTRAP.error("E003 registration is not hash-bound and pre-inference")
rows = {row["cell_id"]: row for row in registration["queue"]}
if bootstrap.cell_id not in rows:
    BOOTSTRAP.error(f"cell is not in the E003 registration: {bootstrap.cell_id}")
row = rows[bootstrap.cell_id]
cell = SimpleNamespace(row=row, cell_id=row["cell_id"], seed=int(row["environment_seed"]), relation=row["relation"], arm="symmetric")
class _Release:
    def release_fingerprint(self, value: Any) -> str:
        return sha256_bytes(canonical_json_bytes({"amendment_id": AMENDMENT_ID, "cell": value.row, "registration_sha256": bootstrap.release_manifest_sha256}))
release = _Release()
...
if (
    not bootstrap.fixture_candidate_sha256
    or not candidate_path.is_file()
    or sha256_file(candidate_path) != bootstrap.fixture_candidate_sha256
):
    BOOTSTRAP.error("fixture candidate digest mismatch")
candidate = json.loads(candidate_path.read_text(encoding="utf-8"))
os.environ["VLA_WAM_V3E003_FIXTURE_CANDIDATE"] = str(candidate_path)
os.environ["VLA_WAM_V3E003_FIXTURE_SHA256"] = bootstrap.fixture_candidate_sha256
...
from experiments.v3.phase_e.bilateral_symmetry_null_control_v3e003.client import V3E003Pi05Client  # noqa: E402
...
TASKS = {
    "left": ("left.py", "V3E003Pi05SymmetricLeftTask"),
    "right": ("right.py", "V3E003Pi05SymmetricRightTask"),
}
task_root = study_root / "experiments/v3/phase_e/bilateral_symmetry_null_control_v3e003/task_files"
registration_order = ("left", "right")
auto_register_droid_envs(
    task=[str(task_root / TASKS[key][0]) for key in registration_order],
    cameras=WRIST_LEFT_RIGHT_HEAD,
)
args_cli.task = [TASKS[cell.relation][1]]
...
class StateCaptureProxy:
    ...
    def _fixture_match(self) -> bool:
        expected = candidate["positions_robot_base_m"]
        robot = self._env.scene["robot"].data
...
def make_client(_: argparse.Namespace) -> V3E003Pi05Client:
    client = V3E003Pi05Client(
