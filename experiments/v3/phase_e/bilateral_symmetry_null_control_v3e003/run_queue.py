...
def main()->None:
 ...
 for row in cells:
  ...
  env=dict(os.environ); env.update({'OMNI_KIT_ACCEPT_EULA':'YES','NVIDIA_DRIVER_CAPABILITIES':'all','VK_ICD_FILENAMES':'/etc/vulkan/icd.d/nvidia_icd.json','VLA_WAM_V3E003_SCENE_ASSET':str(a.repo_root.resolve()/'experiments/v3/phase_e/bilateral_symmetry_null_control_v3e003/scene_assets/rubiks_cube_banana_pair_bowl.usda'),'TMPDIR':f"/tmp/vla_wam_v3e003/{cid.replace(':','__')}"})
  for key in ('TMPDIR',): Path(env[key]).mkdir(parents=True,exist_ok=True)
