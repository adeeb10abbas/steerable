#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R003 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R003":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r003_live_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R003"
    or candidate_schedule.get("status")
    != "frozen_before_any_r003_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r003_live_diagnostic_count") != 0
    or candidate_schedule.get("r003_live_candidate_evaluation_count") != 0
...
r002_predecessor = candidate_schedule.get("r002_predecessor", {})
predecessor_binding = r002_predecessor.get("results", {})
if (
    ...
    != "c9f5dd2d1e8a707bdf14645533b43790ee9015bc7c248cb89bb68a0f58931280"
    or candidate_schedule.get("r002_closure_commit")
    != "27d1bfd844808f7f336bbb4e25552a9c859fd08a"
    or candidate_schedule.get("r002_raw_result", {}).get("sha256")
    != "afb8c3ba2b53f1513bd22fd6135b16cbfe3e4dd9de3fe3d818ac05e458311fe7"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R002 exhaustion")
try:
...
if (
    ...
    or source_push_gate.get("r003_live_diagnostic_count") != 0
...
def _verify_historical_source_rows(schedule: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for pair in schedule["candidate_pairs"]:
        for stage in ("canonical_grasp", "canonical_carry"):
            side = str(pair[stage]["r003_solver_initialization"]["source_side"])
            sources.append(
                (
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r003-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R003",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r003_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r002_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r003_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R003 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    """Run one frozen E004 absolute-IK hold and retain exact convergence evidence."""

    global CURRENT_STAGE
    position = np.asarray(target_position, dtype=np.float64)
    quaternion = _quat_normalize_wxyz(target_quaternion)
    if position.shape != (3,) or not np.all(np.isfinite(position)):
        raise RuntimeError("registered pose-hold target is malformed")
    action = _command_base_link(position, quaternion, 1.0, env.device)
    ...
    for step in range(hold_steps):
        ...
        if step % 10 == 0:
            video_frames.append(
                np.asarray(
                    obs["image_obs"]["head_camera"][0].detach().cpu().numpy(), dtype=np.uint8
                )
            )
    final_window = errors[-required_final_steps:]
    ...
    return {
        ...
        "phase": phase,
        "target_base_link_pose_world_wxyz": np.concatenate((position, quaternion)).tolist(),
        "command_action_8d": _host(action[0]),
        "command_frame_conversion": "target base_link quaternion is sent directly, WXYZ",
...
def _solve_registered_ik(
    ...
    *,
    ...
    initialization = schedule_stage["r003_solver_initialization"]
    ...
    if (
        ...
        raise RuntimeError("R003 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R003 waypoint schedule differs from the registered finite solver")
    ...
    return {
        ...
        "solver_topology": "historical_q_seed_then_eight_registered_abs_ik_waypoints",
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r003_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r003_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R003 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r003_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r003_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r003_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r003-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R003",
                ...
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r003_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R003 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R003 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r003_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r003_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R003 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r003-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R003",
            "status": (
                "passed_r003_state_repair_not_released_for_behavior"
                if passed
                else "r003_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R003 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R003 search: "
