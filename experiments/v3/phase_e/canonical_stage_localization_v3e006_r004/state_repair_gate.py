#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R004 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R004":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r004_live_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R004"
    or candidate_schedule.get("status")
    != "frozen_before_any_r004_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r004_live_diagnostic_count") != 0
    or candidate_schedule.get("r004_live_candidate_evaluation_count") != 0
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R004 residual-correction contract digest differs")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R004 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R004 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R004 waypoint correction binding differs")
...
r003_predecessor = candidate_schedule.get("r003_predecessor", {})
predecessor_binding = r003_predecessor.get("results", {})
if (
    ...
    != "d30ed1dae4f69992cf257b9430fc9165129185d2be9a2af3bc1a2dd9ea5d1261"
    or r003_predecessor.get("closure_commit")
    != "fc2f23a915a98f368181dd897994b67d64a35eeb"
    or r003_predecessor.get("raw_result", {}).get("sha256")
    != "0bac94d3d1e5b93f3eb00f94f1c4a6cc989cbe54d01f2b9247ed6f5ecf5a9392"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R003 diagnostic closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R003 predecessor result is unreadable: {exc}")
...
try:
    validate_r003_diagnostic_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label, expected in predecessor_result["raw_evidence"].items():
    ...
    if (
        ...
        BOOTSTRAP.error(f"R003 predecessor raw evidence changed: {label}")
if source_push_gate.get("status") != "passed_before_first_r004_live_diagnostic_candidate_or_model_request":
    BOOTSTRAP.error("R004 source-push gate did not pass prospectively")
...
if (
    ...
    or source_push_gate.get("r004_live_diagnostic_count") != 0
    or source_push_gate.get("r004_live_candidate_evaluation_count") != 0
    ...
    BOOTSTRAP.error("R004 source-push history counts differ")
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r004-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R004",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r004_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r003_predecessor_results": _binding(args.predecessor_closure_binding),
            "source_push_gate": _binding(args.source_push_gate),
            "residual_correction_source": _binding(
                study_root
                / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r004/residual_correction.py"
            ),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r004_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R004 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R004 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R004 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R004 waypoint schedule differs from the registered finite solver")
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r004_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r004_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R004 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r004_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r004_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r004_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r004-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R004",
                ...
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r004_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R004 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R004 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r004_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r004_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R004 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r004-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R004",
            "status": (
                "passed_r004_state_repair_not_released_for_behavior"
                if passed
                else "r004_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R004 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R004 search: "
