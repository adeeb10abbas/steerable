#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R005 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R005":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r005_live_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R005"
    or candidate_schedule.get("status")
    != "frozen_before_any_r005_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r005_live_diagnostic_count") != 0
    or candidate_schedule.get("r005_live_candidate_evaluation_count") != 0
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R005 residual-correction contract digest differs")
...
if (
    ...
    or repair_registration.get("construction_horizon_contract") != horizon_contract
    or repair_registration.get("construction_horizon_contract_sha256") != horizon_digest
    ...
    BOOTSTRAP.error("R005 construction-horizon contract differs")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R005 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R005 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R005 waypoint correction binding differs")
...
r004_predecessor = candidate_schedule.get("r004_predecessor", {})
predecessor_binding = r004_predecessor.get("results", {})
if (
    ...
    != "aa5eebfd76064b4aa2664b5d296461fd2a8cdbbf489175b3148475f9a2546006"
    or r004_predecessor.get("closure_commit")
    != "4775965d721f2c1e8c875bcec566d7436162cb91"
    or r004_predecessor.get("raw_result", {}).get("sha256")
    != "54c2335c5c4339037bd5f7e7e76ab15c5485191d82de253cc61d27dd66ddb81d"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R004 timeout closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R004 predecessor result is unreadable: {exc}")
...
try:
    validate_r004_timeout_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r005-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R005",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r005_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r004_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r005_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R005's sole lifecycle change before any reset or physics step."""
    ...
    if (
        ...
        raise RuntimeError("registered R005 construction horizon activation changed more than timeout length")
    return {
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R005 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R005 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R005 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R005 waypoint schedule differs from the registered finite solver")
...
def _direct_materialize_and_gate(
    ...
    *,
    ...
    action = _command_base_link(base_position, base_quaternion, 1.0, env.device)
    trace: list[dict[str, Any]] = []
...
def main() -> None:
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r005_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R005 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r005_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            ...
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r005_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r005-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R005",
                ...
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r005_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R005 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R005 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r005_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r005_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R005 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r005-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R005",
            "status": (
                ...
                if passed
                else "r005_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R005 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R005 search: "
