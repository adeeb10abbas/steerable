"""Pure validation of the immutable V3-E006-R005 state-search closure."""
...
def validate_r005_exhaustion_closure(payload: Mapping[str, Any]) -> None:
    expected = {
        "schema_version": "vla-wam-shared-v3e006-r005-state-repair-closure-v2",
        "amendment_id": "V3-E006-R005",
        "status": "r005_candidate_budget_exhausted_no_valid_state_pair",
        "passed": False,
    ...
    for name in (
        ...
        if not isinstance(row, Mapping) or not isinstance(row.get("path"), str):
            raise ValueError(f"R005 binding differs: {name}")
        if not isinstance(row.get("bytes"), int) or isinstance(row.get("bytes"), bool):
            raise ValueError(f"R005 binding bytes differ: {name}")
        digest = row.get("sha256")
        if not isinstance(digest, str) or len(digest) != 64:
            raise ValueError(f"R005 binding digest differs: {name}")
