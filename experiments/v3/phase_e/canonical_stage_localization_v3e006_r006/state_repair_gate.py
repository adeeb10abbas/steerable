#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R006 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R006":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r006_live_diagnostic_candidate_or_model_request":
    BOOTSTRAP.error("repair registration status differs")
if repair_registration.get("counts_at_registration") != {
    "r006_live_diagnostics": 0,
    "r006_live_candidate_evaluations": 0,
    "model_requests": 0,
    "behavioral_episodes": 0,
}:
    BOOTSTRAP.error("repair registration does not bind zero counts")
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R006"
    or candidate_schedule.get("status")
    != "frozen_before_any_r006_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r006_live_diagnostic_count") != 0
    or candidate_schedule.get("r006_live_candidate_evaluation_count") != 0
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R006 residual-correction contract digest differs")
...
if (
    ...
    or repair_registration.get("frozen_inputs", {}).get("r005_candidate_schedule", {}).get("sha256")
    != candidate_schedule.get("r005_target_solver_horizon_schedule", {}).get("sha256")
    ...
    BOOTSTRAP.error("R006 inherited construction-horizon contract differs")
equilibrium_contract = candidate_schedule.get("joint_equilibrium_hold_contract", {})
equilibrium_digest = hashlib.sha256(
    (json.dumps(equilibrium_contract, allow_nan=False, sort_keys=True, separators=(",", ":")) + "\n").encode()
).hexdigest()
if (
    equilibrium_digest != candidate_schedule.get("joint_equilibrium_hold_contract_sha256")
    or repair_registration.get("joint_equilibrium_hold_contract") != equilibrium_contract
    or repair_registration.get("joint_equilibrium_hold_contract_sha256") != equilibrium_digest
    or equilibrium_contract.get("settle_steps") != 780
    or equilibrium_contract.get("registered_construction_max_episode_length_steps") != 900
):
    BOOTSTRAP.error("R006 joint-equilibrium hold contract differs")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R006 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R006 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R006 waypoint correction binding differs")
...
r005_predecessor = candidate_schedule.get("r005_predecessor", {})
predecessor_binding = r005_predecessor.get("results", {})
if (
    ...
    != "550665a234c378cbcb5c8022d16249a980d1a5b5368b08900568c959c51fb9f2"
    or r005_predecessor.get("closure_commit")
    != "040cf75c1d83a2e5f8383d87247fb096e8d2491a"
    or r005_predecessor.get("raw_result", {}).get("sha256")
    != "4cb6c3e7e19e510e2422368131c06074014ba5173caf2134b7d6b456043afc01"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R005 exhaustion closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R005 predecessor result is unreadable: {exc}")
...
try:
    validate_r005_exhaustion_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in (
    ...
    if (
        ...
        BOOTSTRAP.error(f"R005 predecessor raw evidence changed: {label}")
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r006-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R006",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r006_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r005_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r006_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r006-completed-evidence-v1",
        "amendment_id": "V3-E006-R006",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R006's sole lifecycle change before any reset or physics step."""
    ...
    if (
        ...
        raise RuntimeError("registered R006 construction horizon activation changed more than timeout length")
    return {
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R006 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R006 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R006 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R006 waypoint schedule differs from the registered finite solver")
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r006_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r006_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R006 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r006_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r006_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r006_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r006-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R006",
                "status": "r006_known_reachable_diagnostic_failed_candidates_not_evaluated",
                "passed": False,
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r006_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R006 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R006 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r006_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r006_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R006 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r006-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R006",
            "status": (
                ...
                if passed
                else "r006_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R006 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R006 search: "
