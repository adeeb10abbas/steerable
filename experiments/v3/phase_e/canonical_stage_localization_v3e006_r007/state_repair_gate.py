#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R007 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R007":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r007_live_diagnostic_candidate_or_model_request":
    BOOTSTRAP.error("repair registration status differs")
if repair_registration.get("counts_at_registration") != {
    "r007_live_diagnostics": 0,
    "r007_live_candidate_evaluations": 0,
    "model_requests": 0,
    "behavioral_episodes": 0,
}:
    BOOTSTRAP.error("repair registration does not bind zero counts")
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R007"
    or candidate_schedule.get("status")
    != "frozen_before_any_r007_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r007_live_diagnostic_count") != 0
    or candidate_schedule.get("r007_live_candidate_evaluation_count") != 0
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R007 residual-correction contract digest differs")
horizon_contract = candidate_schedule.get("construction_horizon_contract", {})
horizon_digest = hashlib.sha256(
    (json.dumps(horizon_contract, allow_nan=False, sort_keys=True, separators=(",", ":")) + "\n").encode()
).hexdigest()
if (
    horizon_digest != candidate_schedule.get("construction_horizon_contract_sha256")
    or repair_registration.get("frozen_inputs", {}).get("r006_candidate_schedule", {}).get("sha256")
    != candidate_schedule.get("r006_target_solver_horizon_schedule", {}).get("sha256")
    or horizon_contract.get("original_max_episode_length_steps") != 450
    or horizon_contract.get("registered_max_episode_length_steps") != 900
    or horizon_contract.get("worst_case_derivation", {}).get("worst_case_steps") != 795
    or horizon_contract.get("worst_case_derivation", {}).get("registered_margin_steps") != 105
    or horizon_contract.get("behavioral_episode_horizon_unchanged") is not True
):
    BOOTSTRAP.error("R007 inherited construction-horizon contract differs")
open_contact_contract = candidate_schedule.get("open_contact_construction_contract", {})
open_contact_digest = hashlib.sha256(
    (json.dumps(open_contact_contract, allow_nan=False, sort_keys=True, separators=(",", ":")) + "\n").encode()
).hexdigest()
if (
    open_contact_digest != candidate_schedule.get("open_contact_construction_contract_sha256")
    or repair_registration.get("open_contact_construction_contract") != open_contact_contract
    or repair_registration.get("open_contact_construction_contract_sha256") != open_contact_digest
    or open_contact_contract.get("candidate_action_steps") != 810
    or open_contact_contract.get("registered_construction_max_episode_length_steps") != 900
    or open_contact_contract.get("required_episode_length_buf_before_candidate_actions") != 75
):
    BOOTSTRAP.error("R007 open-contact construction contract differs")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R007 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        initialization = pair[stage_name].get("r004_solver_initialization", {})
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R007 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R007 waypoint correction binding differs")
...
r006_predecessor = candidate_schedule.get("r006_predecessor", {})
predecessor_binding = r006_predecessor.get("results", {})
if (
    ...
    != "3c58721d11f669243690aaf3619121d1c348bf788ca56aacd2a009f727065e63"
    or r006_predecessor.get("closure_commit")
    != "125e8f0d231ebd2e3c7d0d9b54dce83e1080cea1"
    or r006_predecessor.get("raw_result", {}).get("sha256")
    != "7eae75c38a7b65ba4b8fbc44f3ca4c565c3af5675134c93570b1dc0e85176011"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R006 exhaustion closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R006 predecessor result is unreadable: {exc}")
...
try:
    validate_r006_exhaustion_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in (
    ...
    if (
        ...
        BOOTSTRAP.error(f"R006 predecessor raw evidence changed: {label}")
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r007-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R007",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r007_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "construction_horizon_contract": horizon_contract,
        "construction_horizon_contract_sha256": horizon_digest,
        ...
        "input_bindings": {
            ...
            "r006_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r007_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r007-completed-evidence-v1",
        "amendment_id": "V3-E006-R007",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R007's sole lifecycle change before any reset or physics step."""
    ...
    cfg.episode_length_s = 900 * step_dt
    ...
    if (
        registered_max_episode_length != 900
        or not math.isclose(registered_episode_length_s, 60.0, rel_tol=0.0, abs_tol=1e-12)
        or before != after
        or int(env.common_step_counter) != common_step_before
        or _host(env.episode_length_buf) != episode_length_before
    ):
        raise RuntimeError("registered R007 construction horizon activation changed more than timeout length")
    return {
        ...
        # R006's inherited waypoint solver required at most 795 steps.  R007's
        # registered candidate path instead consumes the 75-step reset plus
        # all 810 open/contact/lift/settle actions.
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R007 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R007 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R007 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R007 waypoint schedule differs from the registered finite solver")
...
def _open_contact_materialize_and_gate(
    env: Any,
    *,
    stage_name: str,
    schedule_stage: Mapping[str, Any],
    ...
    """Construct the state from exact reset using only normal E004 actions."""

    global CURRENT_STAGE
    contract = candidate_schedule["open_contact_construction_contract"]
    expected_steps = {
        "open_approach": 120,
        "open_descent": 120,
        "normal_close": 90,
    ...
    if contract.get("phase_steps") != expected_steps:
        raise RuntimeError("R007 open-contact phase-step contract differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R007 candidate actions must begin at exact reset step 75")
    targets = schedule_stage["r007_open_contact_targets"]
    approach = targets["approach_base_link_pose"]
    contact = targets["contact_base_link_pose_at_exact_reset_cube"]
    final = targets["stage_target_base_link_pose"]
    ...
    final_position = np.asarray(final["position_world_m"], dtype=np.float64)
    final_quaternion = _quat_normalize_wxyz(final["quaternion_world_wxyz"])
    if not np.all(
        np.isfinite(
            np.concatenate(
                (
        ...
        raise RuntimeError("R007 open-contact target contains a nonfinite value")
    ...
    phases: list[tuple[str, int, np.ndarray, np.ndarray, np.ndarray, np.ndarray, float]] = [
        ...
        ("closed_lift_to_registered_stage_target", 180,
         contact_position, contact_quaternion, final_position, final_quaternion, 1.0),
        ("closed_settle_at_registered_stage_target", 300,
         final_position, final_quaternion, final_position, final_quaternion, 1.0),
    ]
    trace: list[dict[str, Any]] = []
    settled: list[dict[str, Any]] = []
    obs: Mapping[str, Any] | None = None
    ...
    for phase, steps, from_p, from_q, to_p, to_q, grip in phases:
        ...
        for step in range(steps):
            ...
            if phase == "closed_settle_at_registered_stage_target" and step >= 290:
                settled.append(_sample(env, frames, eef_index))
        if termination is not None:
            break
    if termination is not None or obs is None:
        return {
            "passed": False,
            "candidate_rejection": "environment_terminated_during_r007_open_contact_construction",
            "termination": termination,
            "construction_method": "exact_reset_open_approach_normal_close_lift",
            "construction_action_trace": trace,
        }
    if len(trace) != 810 or _host(env.episode_length_buf) != [885]:
        raise RuntimeError("R007 open-contact action/counter total differs")
    return _finalize_unchanged_gates(
        env,
        obs=obs,
        stage_name=stage_name,
        ...
        construction={
            ...
            "candidate_rank": schedule_stage["candidate_rank"],
            "registered_stage_schedule": schedule_stage,
            "open_contact_construction_contract": contract,
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r007_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r007_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R007 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r007_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r007_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r007_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r007-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R007",
                "status": "r007_known_reachable_diagnostic_failed_candidates_not_evaluated",
                "passed": False,
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r007_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R007 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R007 candidate evaluation reached without four passed diagnostics")
        ...
        for stage_name in ("canonical_grasp", "canonical_carry"):
            ...
            try:
                (
                    ...
                    rank=rank, stage_name=stage_name, role="open_contact_materialization"
                )
                ...
                if method != "exact_reset_open_approach_normal_close_lift":
                    raise RuntimeError(f"unregistered construction method: {method}")
                state = _open_contact_materialize_and_gate(
                    material_env,
                    stage_name=stage_name,
                    schedule_stage=schedule_stage,
    ...
    CURRENT_STAGE = "retain_r007_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r007_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R007 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r007-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R007",
            "status": (
                "passed_r007_state_repair_not_released_for_behavior"
                if passed
                else "r007_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R007 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R007 search: "
