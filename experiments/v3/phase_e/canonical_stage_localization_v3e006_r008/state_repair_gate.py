#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R008 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R008":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r008_live_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R008"
    or candidate_schedule.get("status")
    != "frozen_before_any_r008_live_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r008_live_diagnostic_count") != 0
    or candidate_schedule.get("r008_live_candidate_evaluation_count") != 0
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r008.residual_correction import (
    corrected_command,
    validate_contract,
)
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r008.object_servo import (
    object_space_servo_command,
    validate_contract as validate_object_servo_contract,
)
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R008 residual-correction contract digest differs")
object_servo_contract = candidate_schedule.get("object_space_servo_contract", {})
try:
    validate_object_servo_contract(object_servo_contract)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
object_servo_digest = hashlib.sha256(
    (json.dumps(object_servo_contract, allow_nan=False, sort_keys=True, separators=(",", ":")) + "\n").encode()
).hexdigest()
if object_servo_digest != candidate_schedule.get("object_space_servo_contract_sha256"):
    BOOTSTRAP.error("R008 object-space servo contract digest differs")
...
if (
    ...
    BOOTSTRAP.error("R008 joint handoff contract differs")
...
if (
    ...
    or lifecycle_contract.get("worst_case_steps") != 1365
    or lifecycle_contract.get("registered_max_episode_length_steps") != 1500
    or lifecycle_contract.get("registered_margin_steps") != 135
):
    BOOTSTRAP.error("R008 construction lifecycle contract differs")
...
if (
    ...
    BOOTSTRAP.error("R008 predecessor construction contracts are not explicitly archived")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R008 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R008 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R008 waypoint correction binding differs")
...
r007_predecessor = candidate_schedule.get("r007_predecessor", {})
predecessor_binding = r007_predecessor.get("results", {})
if (
    ...
    != "3a6ab612919fd9e5eeef2f4bd030b74c25f6b6f871b6c25c13f92efac5ba9b7d"
    or r007_predecessor.get("closure_commit")
    != "7cc3acc120027bdd181340b443633d8a03d6858d"
    or r007_predecessor.get("raw_result", {}).get("sha256")
    != "f20314dcd32d9d6503dd5c2bd3777a08563fea3ca2a47c5f91bc26f4483a5cd6"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R007 exhaustion closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R007 predecessor result is unreadable: {exc}")
...
try:
    validate_r007_exhaustion_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in ("raw_result", "raw_harness", "raw_launch", "raw_runtime_log", "authoritative_target_validation_receipt"):
    ...
    if (
        ...
        BOOTSTRAP.error(f"R007 predecessor raw evidence changed: {label}")
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r008-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R008",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r008_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r007_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r008_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r008-completed-evidence-v1",
        "amendment_id": "V3-E006-R008",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R008's sole lifecycle change before any reset or physics step."""
    ...
    cfg.episode_length_s = 1500 * step_dt
    ...
    if (
        registered_max_episode_length != 1500
        or not math.isclose(registered_episode_length_s, 100.0, rel_tol=0.0, abs_tol=1e-12)
        or before != after
        or int(env.common_step_counter) != common_step_before
        or _host(env.episode_length_buf) != episode_length_before
    ):
        raise RuntimeError("registered R008 construction horizon activation changed more than timeout length")
    return {
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R008 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R008 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R008 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R008 waypoint schedule differs from the registered finite solver")
...
def _open_contact_materialize_and_gate(
    ...
    *,
    ...
    """Construct with open contact, uniform object servo, then fixed-q settle."""
    ...
    if lifecycle_contract != {
        ...
        raise RuntimeError("R008 lifecycle contract differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R008 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R008 open-contact target contains a nonfinite value")
    ...
    if termination is None:
        ...
        if captured.shape != (13,) or not np.all(np.isfinite(captured)):
            raise RuntimeError("R008 captured-q handoff is malformed")
        ...
        if episode_before_handoff != [765]:
            raise RuntimeError("R008 handoff did not begin at step 765")
    ...
    if len(trace) != 1290 or _host(env.episode_length_buf) != [1365]:
        raise RuntimeError("R008 construction action/counter total differs")
    if captured_joint_target is None or episode_before_handoff is None or len(servo_rows) != 360:
        raise RuntimeError("R008 object servo or q handoff evidence is incomplete")
    return _finalize_unchanged_gates(
        env,
        obs=obs,
        stage_name=stage_name,
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r008_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r008_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R008 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r008_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r008_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r008_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r008-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R008",
                "status": "r008_known_reachable_diagnostic_failed_candidates_not_evaluated",
                "passed": False,
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r008_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R008 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R008 candidate evaluation reached without four passed diagnostics")
        ...
        for stage_name in ("canonical_grasp", "canonical_carry"):
            ...
            try:
                (
                    ...
                    rank=rank, stage_name=stage_name, role="object_servo_q_handoff_materialization"
                )
                ...
                if method != "exact_reset_open_close_uniform_object_servo_q_handoff":
    ...
    CURRENT_STAGE = "retain_r008_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r008_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R008 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r008-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R008",
            "status": (
                "passed_r008_state_repair_not_released_for_behavior"
                if passed
                else "r008_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R008 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R008 search: "
