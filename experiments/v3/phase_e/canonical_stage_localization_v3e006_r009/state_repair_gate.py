#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R009 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R009":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r009_live_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R009"
    ...
    or candidate_schedule.get("r009_live_diagnostic_count") != 0
    or candidate_schedule.get("r009_live_candidate_evaluation_count") != 0
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r009.residual_correction import (
    corrected_command,
    validate_contract,
)
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R009 residual-correction contract digest differs")
...
if pinch_geometry_digest != candidate_schedule.get("pinch_geometry_contract_sha256"):
    BOOTSTRAP.error("R009 pinch-geometry contract digest differs")
...
if (
    ...
    BOOTSTRAP.error("R009 joint handoff contract differs")
...
if (
    ...
    BOOTSTRAP.error("R009 construction lifecycle contract differs")
...
if (
    ...
    BOOTSTRAP.error("R009 predecessor construction contracts are not explicitly archived")
...
if set(construction_asset_bindings) != {"robot_usd", "cube_usd"}:
    BOOTSTRAP.error("R009 collision-geometry asset inventory differs")
for asset_label, expected in construction_asset_bindings.items():
    ...
    if (
        ...
        BOOTSTRAP.error(
            f"R009 collision-geometry asset binding changed: {asset_label}: {asset_path}"
        )
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R009 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R009 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R009 waypoint correction binding differs")
...
r008_predecessor = candidate_schedule.get("r008_predecessor", {})
predecessor_binding = r008_predecessor.get("results", {})
if (
    ...
    != "5f86b77b721806b0c19416b622e9a6aab68b13de6d596a6dadec89ed41a081d5"
    or r008_predecessor.get("closure_commit")
    != "a13d5b7f8a4be6374a22483a436b9a41aadd1c9f"
    or r008_predecessor.get("raw_result", {}).get("sha256")
    != "58f8e54994348634d8793f0448c49144b35ab3b601065ab4be298cbd9ebc6548"
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R008 exhaustion closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R008 predecessor result is unreadable: {exc}")
...
try:
    validate_r008_exhaustion_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in ("raw_result", "raw_harness", "raw_launch", "raw_runtime_log", "authoritative_target_validation_receipt"):
    ...
    if (
        ...
        BOOTSTRAP.error(f"R008 predecessor raw evidence changed: {label}")
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r009.pinch_geometry import (  # noqa: E402
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r009-state-repair-attempt-v1",
        "study_id": "vla_wam_language_steerability_v3",
        "amendment_id": "V3-E006-R009",
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "state_candidate_count": state_candidate_count,
        "r009_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r008_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r009_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r009-completed-evidence-v1",
        "amendment_id": "V3-E006-R009",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R009's sole lifecycle change before any reset or physics step."""
    ...
    if (
        ...
        raise RuntimeError("registered R009 construction horizon activation changed more than timeout length")
    return {
...
def _valid_descendants(root_path: str) -> list[Any]:
    ...
    if not root or not root.IsValid():
        raise RuntimeError(f"R009 geometry root prim is invalid: {root_path}")
...
def _resolve_unique_inner_finger_body(
    *, robot_root_path: str, suffix: str
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R009 expected exactly one {suffix} body under {robot_root_path}; "
            f"found {matches}"
        )
    return matches[0]
...
def _enabled_collision_prim_paths(root_path: str) -> list[str]:
    ...
    matches.sort()
    if not matches:
        raise RuntimeError(f"R009 found no enabled collision prim under {root_path}")
    return matches
...
def _collision_geometry_body_local(
    *, collision_prim_paths: Sequence[str], body_prim_path: str
    ...
    if not body or not body.IsValid():
        raise RuntimeError(f"R009 bound rigid-body prim changed: {body_prim_path}")
    if not body.HasAPI(UsdPhysics.RigidBodyAPI):
        raise RuntimeError(f"R009 collision owner lacks RigidBodyAPI: {body_prim_path}")
    ...
    for path in collision_prim_paths:
        ...
        if not prim or not prim.IsValid() or not prim.HasAPI(UsdPhysics.CollisionAPI):
            raise RuntimeError(f"R009 bound collision prim changed: {path}")
        cursor = prim
        ...
        if owning_rigid_body_paths != [body_prim_path]:
            raise RuntimeError(
                "R009 collision crosses a nested/different rigid-body boundary: "
        ...
        if UsdPhysics.CollisionAPI(prim).GetCollisionEnabledAttr().Get() is False:
            raise RuntimeError(f"R009 bound collision prim became disabled: {path}")
        aligned = bbox_cache.ComputeLocalBound(prim).ComputeAlignedRange()
        minimum = np.asarray(
        ...
        if (
            ...
            raise RuntimeError(f"R009 collision prim has invalid local bounds: {path}")
        prim_world = xform_cache.GetLocalToWorldTransform(prim)
        corners_body = []
        for corner in _range_corners(minimum, maximum):
            point = world_body.Transform(prim_world.Transform(corner))
            row = [float(point[axis]) for axis in range(3)]
            if not np.all(np.isfinite(row)):
                raise RuntimeError(f"R009 collision corner is nonfinite: {path}")
            corners_body.append(row)
            aggregate.append(row)
        shapes.append(
            {
                "collision_prim_path": path,
                "collision_prim_local_minimum_m": minimum.tolist(),
                "collision_prim_local_maximum_m": maximum.tolist(),
    ...
    if corners.ndim != 2 or corners.shape[0] < 8 or corners.shape[1] != 3:
        raise RuntimeError("R009 aggregate body-local collision geometry is empty")
    ...
    value = {
        ...
        "derivation": (
            "one-time USD local collision bounds transformed into owning rigid-body "
            "coordinates before candidate actions; never polled dynamically"
        ),
    }
...
def _unique_robot_body_tensor_index(env: Any, body_name: str) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R009 expected one tensor body named {body_name}; found {matches} in {names}"
        )
    return matches[0]
...
def _tensor_pose_evidence(
    *, raw_world_position: Sequence[float], quaternion: Sequence[float], env_origin: Sequence[float]
    ...
    if raw.shape != (3,) or origin.shape != (3,) or not np.all(np.isfinite(np.concatenate((raw, origin, q)))):
        raise RuntimeError("R009 tensor pose is malformed")
    return {
...
def _resolve_pinch_scene_geometry(env: Any) -> dict[str, Any]:
    ...
    if left_body == right_body:
        raise RuntimeError("R009 inner-finger body resolution is not unique")
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R009 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R009 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R009 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R009 waypoint schedule differs from the registered finite solver")
...
def _archived_r008_open_contact_materialize_and_gate_never_called(
    ...
    *,
    ...
    """Archived predecessor topology; R009 dispatch cannot call this function."""

    raise RuntimeError("archived R008 object-servo construction is unreachable in R009")
    ...
    if lifecycle_contract != {
        ...
        raise RuntimeError("R009 lifecycle contract differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R009 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R009 open-contact target contains a nonfinite value")
    ...
    if termination is None:
        ...
        if captured.shape != (13,) or not np.all(np.isfinite(captured)):
            raise RuntimeError("R009 captured-q handoff is malformed")
        ...
        if episode_before_handoff != [765]:
            raise RuntimeError("R009 handoff did not begin at step 765")
    ...
    if len(trace) != 1290 or _host(env.episode_length_buf) != [1365]:
        raise RuntimeError("R009 construction action/counter total differs")
    if captured_joint_target is None or episode_before_handoff is None or len(servo_rows) != 360:
        raise RuntimeError("R009 object servo or q handoff evidence is incomplete")
    return _finalize_unchanged_gates(
        env,
        obs=obs,
        stage_name=stage_name,
...
def _pinch_geometry_materialize_and_gate(
    ...
    *,
    ...
    if registered_lifecycle != expected_lifecycle:
        raise RuntimeError("R009 construction lifecycle contract differs at materialization")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R009 candidate actions must begin at exact reset step 75")

    target_cube = schedule_stage["r009_target_cube_pose"]
    target_cube_position = np.asarray(target_cube["position_world_m"], dtype=np.float64)
    target_cube_quaternion = _quat_normalize_wxyz(
        target_cube["quaternion_world_wxyz"]
    )
    acquisition_quaternion = _quat_normalize_wxyz(
        schedule_stage["r009_acquisition_base_quaternion_world_wxyz"]
    )
    final_quaternion = _quat_normalize_wxyz(
        schedule_stage["r009_final_base_quaternion_world_wxyz"]
    )
    if not np.all(
        np.isfinite(
            np.concatenate(
        ...
        raise RuntimeError("R009 frozen geometry target contains a nonfinite value")
    ...
    if (
        ...
        raise RuntimeError("R009 cube collision-center reconstruction is invalid")
    ...
    def run_cartesian_step(
        *,
        ...
        if bool(terminated[0]) or bool(truncated[0]):
            ...
            raise RuntimeError(
                "R009 construction environment terminated; registered candidate "
    ...
    if episode_before_handoff != [1095]:
        raise RuntimeError("R009 captured-q handoff did not begin at registered step 1095")
    ...
    for step in range(600):
        ...
        if termination is not None:
            raise RuntimeError(
                "R009 construction environment terminated during captured-q settle"
            )
    ...
    if obs is None or len(trace) != 1620 or len(acquisition_trace) != 1020:
        raise RuntimeError("R009 retained action trace length differs")
    if len(contact_trace) != 1620 or _host(env.episode_length_buf) != [1695]:
        raise RuntimeError("R009 contact trace or lifecycle counter differs")
    if len(settled) != 10:
        raise RuntimeError("R009 final scientific gate window differs")
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r009_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r009_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R009 failure: "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r009_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r009_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r009_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                "schema_version": "vla-wam-shared-v3e006-r009-state-repair-result-v1",
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R009",
                ...
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "r009_live_diagnostic_count": len(diagnostics),
                ...
                "release_boundary": (
                    "registered diagnostic failure blocks every R009 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R009 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r009_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r009_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R009 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r009-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R009",
            "status": (
                ...
                if passed
                else "r009_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R009 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R009 search: "
