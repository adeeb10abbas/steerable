"""Pure geometric pinch alignment and fixed transport math for V3-E006-R011."""
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.object_servo import (
...
def validate_contract(value: Mapping[str, Any]) -> None:
    ...
    for key, expected_value in expected.items():
        if value.get(key) != expected_value:
            raise ValueError(f"R011 pinch geometry contract differs: {key}")
...
def validate_attachment_preflight_contract(value: Mapping[str, Any]) -> None:
    ...
    if dict(value) != expected:
        ...
        raise ValueError(f"R011 geometry attachment preflight contract differs: {key}")
