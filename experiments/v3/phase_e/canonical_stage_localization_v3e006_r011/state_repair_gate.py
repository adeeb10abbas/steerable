#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R011 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R011":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r011_live_preflight_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R011"
    or candidate_schedule.get("status")
    != "frozen_before_any_r011_live_preflight_diagnostic_candidate_or_model_request"
    or candidate_schedule.get("candidate_budget") != 4
    or candidate_schedule.get("diagnostic_budget") != 4
    or candidate_schedule.get("r011_geometry_attachment_preflight_count") != 0
    or candidate_schedule.get("r011_live_diagnostic_count") != 0
    or candidate_schedule.get("r011_live_candidate_evaluation_count") != 0
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.residual_correction import (
    corrected_command,
    validate_contract,
)
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.predecessor_contract import (
    R010_CLOSURE_COMMIT,
    R010_RAW_PREFLIGHT_SHA256,
    R010_RAW_RESULT_SHA256,
    R010_RESULTS_SHA256,
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R011 residual-correction contract digest differs")
...
if pinch_geometry_digest != candidate_schedule.get("pinch_geometry_contract_sha256"):
    BOOTSTRAP.error("R011 pinch-geometry contract digest differs")
...
if geometry_attachment_preflight_digest != candidate_schedule.get(
    ...
    BOOTSTRAP.error("R011 geometry-attachment preflight contract digest differs")
...
if (
    ...
    BOOTSTRAP.error("R011 joint handoff contract differs")
...
if (
    ...
    BOOTSTRAP.error("R011 construction lifecycle contract differs")
...
if (
    ...
    != "archived_lineage_only_not_active_r011_runtime_evidence"
    ...
    BOOTSTRAP.error("R011 predecessor construction contracts are not explicitly archived")
...
if set(construction_asset_bindings) != {"robot_usd", "cube_usd"}:
    BOOTSTRAP.error("R011 collision-geometry asset inventory differs")
for asset_label, expected in construction_asset_bindings.items():
    ...
    if (
        ...
        BOOTSTRAP.error(
            f"R011 collision-geometry asset binding changed: {asset_label}: {asset_path}"
        )
scene_sync_source_bindings = candidate_schedule.get(
    "scene_sync_source_bindings", {}
)
if set(scene_sync_source_bindings) != {
    ...
    BOOTSTRAP.error("R011 scene-sync source inventory differs")
for source_label, expected in scene_sync_source_bindings.items():
    ...
    if (
        ...
        BOOTSTRAP.error(
            f"R011 scene-sync source binding changed: {source_label}: {source_path}"
        )
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R011 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if (
            ...
            BOOTSTRAP.error(
                "R011 retained runtime-driving aliases differ from immutable R010/R009"
            )
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R011 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R011 waypoint correction binding differs")
...
r010_predecessor = candidate_schedule.get("r010_predecessor", {})
predecessor_binding = r010_predecessor.get("results", {})
if (
    ...
    or predecessor_binding.get("sha256") != R010_RESULTS_SHA256
    or r010_predecessor.get("closure_commit") != R010_CLOSURE_COMMIT
    or r010_predecessor.get("raw_result", {}).get("sha256")
    != R010_RAW_RESULT_SHA256
    or r010_predecessor.get("raw_preflight", {}).get("sha256")
    != R010_RAW_PREFLIGHT_SHA256
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R010 oracle-failure closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R010 predecessor result is unreadable: {exc}")
try:
    validate_r010_oracle_failure_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in ("raw_result", "raw_preflight", "raw_harness", "raw_launch", "raw_runtime_log", "authoritative_target_validation_receipt"):
    ...
    if (
        ...
        BOOTSTRAP.error(f"R010 predecessor raw evidence changed: {label}")
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.source_gate_contract import (
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.pinch_geometry import (  # noqa: E402
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r011-state-repair-attempt-v1",
        ...
        "r011_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r010_predecessor_results": _binding(args.predecessor_closure_binding),
            ...
            "scene_sync_sources": {
                ...
                for name, value in scene_sync_source_bindings.items()
            },
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r011_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r011-completed-evidence-v1",
        "amendment_id": "V3-E006-R011",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R011's sole lifecycle change before any reset or physics step."""
    ...
    if (
        ...
        raise RuntimeError("registered R011 construction horizon activation changed more than timeout length")
    return {
...
def _valid_descendants(root_path: str) -> list[Any]:
    ...
    if not root or not root.IsValid():
        raise RuntimeError(f"R011 geometry root prim is invalid: {root_path}")
...
def _resolve_unique_inner_finger_body(
    *, robot_root_path: str, suffix: str
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R011 expected exactly one {suffix} body under {robot_root_path}; "
            f"found {matches}"
        )
    return matches[0]
...
def _enabled_collision_prim_paths(root_path: str) -> list[str]:
    ...
    matches.sort()
    if not matches:
        raise RuntimeError(f"R011 found no enabled collision prim under {root_path}")
    return matches
...
def _collision_geometry_body_local(
    *, collision_prim_paths: Sequence[str], body_prim_path: str
    ...
    if not body or not body.IsValid():
        raise RuntimeError(f"R011 bound rigid-body prim changed: {body_prim_path}")
    if not body.HasAPI(UsdPhysics.RigidBodyAPI):
        raise RuntimeError(f"R011 collision owner lacks RigidBodyAPI: {body_prim_path}")
    ...
    for path in collision_prim_paths:
        ...
        if not prim or not prim.IsValid() or not prim.HasAPI(UsdPhysics.CollisionAPI):
            raise RuntimeError(f"R011 bound collision prim changed: {path}")
        cursor = prim
        ...
        if owning_rigid_body_paths != [body_prim_path]:
            raise RuntimeError(
                "R011 collision crosses a nested/different rigid-body boundary: "
        ...
        if UsdPhysics.CollisionAPI(prim).GetCollisionEnabledAttr().Get() is False:
            raise RuntimeError(f"R011 bound collision prim became disabled: {path}")
        ...
        # only body-local geometry retained by R011.  In particular, do not
        ...
        if (
            ...
            raise RuntimeError(f"R011 collision prim has invalid body-relative bounds: {path}")
        corners_body = []
        for corner in _range_corners(minimum, maximum):
            row = [float(corner[axis]) for axis in range(3)]
            if not np.all(np.isfinite(row)):
                raise RuntimeError(f"R011 collision corner is nonfinite: {path}")
    ...
    if corners.ndim != 2 or corners.shape[0] < 8 or corners.shape[1] != 3:
        raise RuntimeError("R011 aggregate body-local collision geometry is empty")
...
def _unique_robot_body_tensor_index(env: Any, body_name: str) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R011 expected one tensor body named {body_name}; found {matches} in {names}"
        )
    return matches[0]
...
def _tensor_pose_evidence(
    *, raw_world_position: Sequence[float], quaternion: Sequence[float], env_origin: Sequence[float]
    ...
    if raw.shape != (3,) or origin.shape != (3,) or not np.all(np.isfinite(np.concatenate((raw, origin, q)))):
        raise RuntimeError("R011 tensor pose is malformed")
    return {
...
def _resolve_pinch_scene_geometry(env: Any) -> dict[str, Any]:
    ...
    if left_body == right_body:
        raise RuntimeError("R011 inner-finger body resolution is not unique")
...
def _run_geometry_attachment_preflight(
    ...
    *,
    ...
    """Run the one frozen scene-specific tensor-to-USD attachment oracle."""
    ...
    if setting_path != str(SETTING_UPDATE_TO_USD):
        raise RuntimeError("R011 PhysX update-to-USD setting constant differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R011 geometry oracle did not run after exact 75-step reset")
    origin = np.asarray(_host(env.scene.env_origins[0]), dtype=np.float64)
    if origin.shape != (3,) or not np.all(np.isfinite(origin)):
        raise RuntimeError("R011 geometry oracle env origin is malformed")
    simulation_context = SimulationContext.instance()
    if simulation_context is None:
        raise RuntimeError("R011 SimulationContext singleton is unavailable")
    if env.sim is not simulation_context:
        raise RuntimeError("R011 fresh environment uses a different SimulationContext")
    initial_stage = simulation_context.get_initial_stage()
    current_stage_before = omni.usd.get_context().get_stage()
    if initial_stage is None or current_stage_before is None:
        raise RuntimeError("R011 USD stage is unavailable")
    if initial_stage is not current_stage_before:
        raise RuntimeError("R011 initial/current USD stage object identity differs")
    stage_cache = UsdUtils.StageCache.Get()
    stage_id_before_object = stage_cache.GetId(current_stage_before)
    if not stage_id_before_object.IsValid():
        raise RuntimeError("R011 current USD stage has no valid stage-cache ID")
    stage_id_before = int(stage_id_before_object.ToLongInt())
    if stage_id_before == 0:
        raise RuntimeError("R011 current USD stage-cache ID is zero")
    root_layer_before = str(current_stage_before.GetRootLayer().identifier)
    configured_scene_path = str(simulation_context.cfg.physics_prim_path)
    if configured_scene_path != str(
        geometry_attachment_preflight_contract[
            "expected_configured_physics_scene_path"
        ]
    ):
        raise RuntimeError("R011 configured PhysicsScene path differs from registration")
    scene_paths = sorted(
        str(prim.GetPath())
        for prim in current_stage_before.Traverse()
        if prim.IsValid() and prim.IsA(UsdPhysics.Scene)
    )
    if scene_paths != [configured_scene_path]:
        raise RuntimeError(
            "R011 expected exactly the configured PhysicsScene; "
            f"configured={configured_scene_path} observed={scene_paths}"
        )
    scene_path_int = int(PhysicsSchemaTools.sdfPathToInt(configured_scene_path))
    if scene_path_int == 0:
        raise RuntimeError("R011 configured PhysicsScene encoded to zero")
    ...
    update_transformations_scene = getattr(
        physx_interface, "update_transformations_scene", None
    )
    if not callable(update_transformations_scene):
        raise RuntimeError("R011 scene-specific PhysX-to-USD API is unavailable")
    update_transformations_scene(scene_path_int, True, False)
    setting_after = bool(settings.get_as_bool(setting_path))
    if setting_after != setting_before:
        raise RuntimeError("R011 one-shot PhysX-to-USD sync mutated persistent setting")
    stage = omni.usd.get_context().get_stage()
    if stage is not current_stage_before or stage is not initial_stage:
        raise RuntimeError("R011 USD stage object changed during scene-specific sync")
    stage_id_after_object = stage_cache.GetId(stage)
    if not stage_id_after_object.IsValid():
        raise RuntimeError("R011 post-sync USD stage has no valid stage-cache ID")
    stage_id_after = int(stage_id_after_object.ToLongInt())
    root_layer_after = str(stage.GetRootLayer().identifier)
    if stage_id_after != stage_id_before or root_layer_after != root_layer_before:
        raise RuntimeError("R011 USD stage identity changed during scene-specific sync")
    ...
    for role in ("left", "right", "cube"):
        ...
        if not body_prim or not body_prim.IsValid():
            raise RuntimeError(f"R011 geometry oracle body prim changed: {body_path}")
    ...
    for role in ("left", "right", "cube"):
        ...
        for shape in static["shape_local_geometry"]:
            ...
            if not collision_prim or not collision_prim.IsValid():
                raise RuntimeError(
                    f"R011 geometry oracle collision prim changed: {collision_path}"
                )
            ...
            if not np.all(np.isfinite(all_values)):
                raise RuntimeError(
                    f"R011 geometry oracle produced nonfinite bounds: {collision_path}"
                )
            minimum_error = float(
    ...
    return {
        "schema_version": (
            "vla-wam-shared-v3e006-r011-geometry-attachment-preflight-v1"
        ),
        "status": (
            "passed_r011_scene_specific_relative_bound_tensor_world_oracle_preflight"
            if passed
            else "failed_r011_scene_specific_relative_bound_tensor_world_oracle_preflight"
        ),
        "passed": passed,
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        ...
        "physics_to_usd_synchronization_call": (
            "omni.physx.get_physx_interface().update_transformations_scene"
            "(scene_path_int, True, False)"
        ),
        "physics_to_usd_synchronization_call_arguments": {
            "physics_scene_path": configured_scene_path,
            "physics_scene_path_integer": scene_path_int,
            "update_to_usd": True,
            "update_velocities_to_usd": False,
        },
        "physics_to_usd_synchronization_call_count": 1,
        ...
        "stage_and_scene_identity": {
            ...
            "environment_sim_is_simulation_context_singleton": True,
            "initial_stage_is_current_stage_before_sync": True,
            "initial_stage_is_current_stage_after_sync": True,
            "stage_cache_id_before": stage_id_before,
            "stage_cache_id_after": stage_id_after,
            "stage_cache_id_valid_nonzero_and_unchanged": (
                stage_id_before != 0 and stage_id_after == stage_id_before
            ),
            "root_layer_identifier_before": root_layer_before,
            "root_layer_identifier_after": root_layer_after,
            "root_layer_identifier_unchanged": root_layer_after == root_layer_before,
            "configured_physics_scene_path": configured_scene_path,
            "resolved_physics_scene_paths": scene_paths,
            "unique_configured_physics_scene": scene_paths == [configured_scene_path],
            "physics_scene_path_integer": scene_path_int,
            "physics_scene_path_integer_api": (
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R011 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R011 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R011 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R011 waypoint schedule differs from the registered finite solver")
...
def _archived_r008_open_contact_materialize_and_gate_never_called(
    ...
    *,
    ...
    """Archived predecessor topology; R011 dispatch cannot call this function."""

    raise RuntimeError("archived R008 object-servo construction is unreachable in R011")
    ...
    if lifecycle_contract != {
        ...
        raise RuntimeError("R011 lifecycle contract differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R011 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R011 open-contact target contains a nonfinite value")
    ...
    if termination is None:
        ...
        if captured.shape != (13,) or not np.all(np.isfinite(captured)):
            raise RuntimeError("R011 captured-q handoff is malformed")
        ...
        if episode_before_handoff != [765]:
            raise RuntimeError("R011 handoff did not begin at step 765")
    ...
    if len(trace) != 1290 or _host(env.episode_length_buf) != [1365]:
        raise RuntimeError("R011 construction action/counter total differs")
    if captured_joint_target is None or episode_before_handoff is None or len(servo_rows) != 360:
        raise RuntimeError("R011 object servo or q handoff evidence is incomplete")
    return _finalize_unchanged_gates(
        env,
        obs=obs,
        stage_name=stage_name,
...
def _pinch_geometry_materialize_and_gate(
    ...
    *,
    ...
    if registered_lifecycle != expected_lifecycle:
        raise RuntimeError("R011 construction lifecycle contract differs at materialization")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R011 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R011 frozen geometry target contains a nonfinite value")
    ...
    if (
        ...
        raise RuntimeError(
            "R011 rank-stage collision inventory/body-relative geometry differs "
    ...
    if (
        ...
        raise RuntimeError("R011 cube collision-center reconstruction is invalid")
    ...
    def run_cartesian_step(
        *,
        ...
        if bool(terminated[0]) or bool(truncated[0]):
            ...
            raise RuntimeError(
                "R011 construction environment terminated; registered candidate "
    ...
    if episode_before_handoff != [1095]:
        raise RuntimeError("R011 captured-q handoff did not begin at registered step 1095")
    ...
    for step in range(600):
        ...
        if termination is not None:
            raise RuntimeError(
                "R011 construction environment terminated during captured-q settle"
            )
    ...
    if obs is None or len(trace) != 1620 or len(acquisition_trace) != 1020:
        raise RuntimeError("R011 retained action trace length differs")
    if len(contact_trace) != 1620 or _host(env.episode_length_buf) != [1695]:
        raise RuntimeError("R011 contact trace or lifecycle counter differs")
    if len(settled) != 10:
        raise RuntimeError("R011 final scientific gate window differs")
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r011_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r011_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R011 failure: "
    ...
    # R011 first validates that its one-time body-relative extraction attaches
    ...
    try:
        (
            ...
            role="scene_specific_relative_bound_tensor_world_oracle_preflight",
        )
        CURRENT_STAGE = "r011_geometry_attachment_preflight_resolve_relative_bounds"
        preflight_geometry = _resolve_pinch_scene_geometry(preflight_env)
        CURRENT_STAGE = "r011_geometry_attachment_preflight_one_shot_usd_oracle"
    ...
    if GEOMETRY_ATTACHMENT_PREFLIGHT is None:
        raise RuntimeError("R011 geometry attachment preflight produced no receipt")
    ...
    if GEOMETRY_ATTACHMENT_PREFLIGHT["passed"] is not True:
        CURRENT_STAGE = "r011_geometry_attachment_preflight_failed_blocking_candidates"
        video_path = (
            args.output_dir
            / "videos"
            / "v3e006_r011_geometry_attachment_preflight_failed.mp4"
        )
        video_path.parent.mkdir(parents=True)
        _write_video(video_path, video_frames)
        report = {
            "schema_version": "vla-wam-shared-v3e006-r011-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R011",
            ...
            "r010_predecessor_results": _binding(
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r011_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r011_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r011_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                ...
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R011",
                "status": "r011_known_reachable_diagnostic_failed_candidates_not_evaluated",
                "passed": False,
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "geometry_attachment_preflight_count": 1,
                "r011_live_diagnostic_count": len(diagnostics),
                "repair_candidate_evaluation_count": 0,
                "state_candidate_count": 0,
                "candidate_budget": 4,
                "diagnostic_budget": 4,
                ...
                "video": _binding(video_path),
                "release_boundary": (
                    "registered diagnostic failure blocks every R011 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R011 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r011_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r011_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R011 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r011-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R011",
            "status": (
                "passed_r011_state_repair_not_released_for_behavior"
                if passed
                else "r011_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R011 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R011 search: "
