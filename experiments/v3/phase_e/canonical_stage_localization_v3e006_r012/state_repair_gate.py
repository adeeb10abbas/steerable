#!/usr/bin/env python3
"""Evaluate the preregistered V3-E006-R012 contact-consistent repair schedule.
...
if repair_registration.get("repair_amendment_id") != "V3-E006-R012":
    BOOTSTRAP.error("repair registration ID differs")
if repair_registration.get("status") != "prospectively_registered_before_any_r012_live_preflight_diagnostic_candidate_or_model_request":
...
if (
    candidate_schedule.get("repair_amendment_id") != "V3-E006-R012"
    ...
    or candidate_schedule.get("r012_geometry_attachment_preflight_count") != 0
    or candidate_schedule.get("r012_live_diagnostic_count") != 0
    or candidate_schedule.get("r012_live_candidate_evaluation_count") != 0
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r012.residual_correction import (
    corrected_command,
    validate_contract,
)
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r012.predecessor_contract import (
    R011_CLOSURE_COMMIT,
    R011_RAW_PREFLIGHT_SHA256,
    R011_RAW_RESULT_SHA256,
    R011_RESULTS_SHA256,
...
if correction_digest != candidate_schedule.get("residual_correction_contract_sha256"):
    BOOTSTRAP.error("R012 residual-correction contract digest differs")
...
if pinch_geometry_digest != candidate_schedule.get("pinch_geometry_contract_sha256"):
    BOOTSTRAP.error("R012 pinch-geometry contract digest differs")
...
if geometry_attachment_preflight_digest != candidate_schedule.get(
    ...
    BOOTSTRAP.error("R012 geometry-attachment preflight contract digest differs")
...
if (
    ...
    BOOTSTRAP.error("R012 joint handoff contract differs")
...
if (
    ...
    BOOTSTRAP.error("R012 construction lifecycle contract differs")
...
if (
    ...
    != "archived_lineage_only_not_active_r012_runtime_evidence"
    ...
    BOOTSTRAP.error("R012 predecessor construction contracts are not explicitly archived")
...
if set(construction_asset_bindings) != {"robot_usd", "cube_usd"}:
    BOOTSTRAP.error("R012 collision-geometry asset inventory differs")
for asset_label, expected in construction_asset_bindings.items():
    ...
    if (
        ...
        BOOTSTRAP.error(
            f"R012 collision-geometry asset binding changed: {asset_label}: {asset_path}"
        )
if "scene_sync_source_bindings" in candidate_schedule:
    BOOTSTRAP.error("R012 forbids dynamic USD scene-sync sources")
for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
    if (
        ...
        BOOTSTRAP.error("R012 diagnostic correction binding differs")
for pair in candidate_schedule["candidate_pairs"]:
    for stage_name in ("canonical_grasp", "canonical_carry"):
        ...
        if (
            ...
            BOOTSTRAP.error(
                "R012 retained runtime-driving aliases differ from immutable R010/R009"
            )
        ...
        if initialization.get("residual_correction_contract_sha256") != correction_digest:
            BOOTSTRAP.error("R012 candidate correction binding differs")
        for waypoint in initialization.get("waypoints", []):
            if (
                ...
                BOOTSTRAP.error("R012 waypoint correction binding differs")
...
r011_predecessor = candidate_schedule.get("r011_predecessor", {})
predecessor_binding = r011_predecessor.get("results", {})
if (
    ...
    or predecessor_binding.get("sha256") != R011_RESULTS_SHA256
    or r011_predecessor.get("closure_commit") != R011_CLOSURE_COMMIT
    or r011_predecessor.get("raw_result", {}).get("sha256")
    != R011_RAW_RESULT_SHA256
    or r011_predecessor.get("raw_preflight", {}).get("sha256")
    != R011_RAW_PREFLIGHT_SHA256
):
    BOOTSTRAP.error("candidate schedule does not bind the immutable R011 scene-sync-failure closure")
try:
    predecessor_result = json.loads(args.predecessor_closure_binding.read_text(encoding="utf-8"))
except (OSError, json.JSONDecodeError) as exc:
    BOOTSTRAP.error(f"R011 predecessor result is unreadable: {exc}")
try:
    validate_r011_scene_sync_failure_closure(predecessor_result)
except ValueError as exc:
    BOOTSTRAP.error(str(exc))
for label in ("raw_result", "raw_preflight", "raw_harness", "raw_launch", "raw_runtime_log", "authoritative_target_validation_receipt"):
    ...
    if (
        ...
        BOOTSTRAP.error(f"R011 predecessor raw evidence changed: {label}")
...
from experiments.v3.phase_e.canonical_stage_localization_v3e006_r012.pinch_geometry import (  # noqa: E402
...
def _base_evidence(*, candidate_gate_passed: bool = False, state_candidate_count: int = 0) -> dict[str, Any]:
    ...
    return {
        "schema_version": "vla-wam-shared-v3e006-r012-state-repair-attempt-v1",
        ...
        "r012_live_diagnostic_count": DIAGNOSTIC_EVALUATION_COUNT,
        ...
        "input_bindings": {
            ...
            "r011_predecessor_results": _binding(args.predecessor_closure_binding),
...
def _write_failure(exc: BaseException) -> Path:
    ...
    report = {
        **_base_evidence(),
        "status": "infrastructure_invalid_r012_state_repair",
        "passed": False,
        "failure_exit_policy": (
...
def _persist_completed_evidence(
    *, diagnostics: list[dict[str, Any]], attempts: list[dict[str, Any]]
    ...
    payload = {
        "schema_version": "vla-wam-shared-v3e006-r012-completed-evidence-v1",
        "amendment_id": "V3-E006-R012",
...
def _activate_registered_construction_horizon(env: Any, cfg: Any) -> dict[str, Any]:
    """Apply R012's sole lifecycle change before any reset or physics step."""
    ...
    if (
        ...
        raise RuntimeError("registered R012 construction horizon activation changed more than timeout length")
    return {
...
def _valid_descendants(root_path: str) -> list[Any]:
    ...
    if not root or not root.IsValid():
        raise RuntimeError(f"R012 geometry root prim is invalid: {root_path}")
...
def _resolve_unique_inner_finger_body(
    *, robot_root_path: str, suffix: str
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R012 expected exactly one {suffix} body under {robot_root_path}; "
            f"found {matches}"
        )
    return matches[0]
...
def _enabled_collision_prim_paths(root_path: str) -> list[str]:
    ...
    matches.sort()
    if not matches:
        raise RuntimeError(f"R012 found no enabled collision prim under {root_path}")
    return matches
...
def _collision_geometry_body_local(
    *, collision_prim_paths: Sequence[str], body_prim_path: str
    ...
    if not body or not body.IsValid():
        raise RuntimeError(f"R012 bound rigid-body prim changed: {body_prim_path}")
    if not body.HasAPI(UsdPhysics.RigidBodyAPI):
        raise RuntimeError(f"R012 collision owner lacks RigidBodyAPI: {body_prim_path}")
    ...
    for path in collision_prim_paths:
        ...
        if not prim or not prim.IsValid() or not prim.HasAPI(UsdPhysics.CollisionAPI):
            raise RuntimeError(f"R012 bound collision prim changed: {path}")
        cursor = prim
        ...
        if owning_rigid_body_paths != [body_prim_path]:
            raise RuntimeError(
                "R012 collision crosses a nested/different rigid-body boundary: "
        ...
        if UsdPhysics.CollisionAPI(prim).GetCollisionEnabledAttr().Get() is False:
            raise RuntimeError(f"R012 bound collision prim became disabled: {path}")
        ...
        # only body-local geometry retained by R012.  In particular, do not
        ...
        if (
            ...
            raise RuntimeError(f"R012 collision prim has invalid body-relative bounds: {path}")
        corners_body = []
        for corner in _range_corners(minimum, maximum):
            row = [float(corner[axis]) for axis in range(3)]
            if not np.all(np.isfinite(row)):
                raise RuntimeError(f"R012 collision corner is nonfinite: {path}")
    ...
    if corners.ndim != 2 or corners.shape[0] < 8 or corners.shape[1] != 3:
        raise RuntimeError("R012 aggregate body-local collision geometry is empty")
...
def _unique_robot_body_tensor_index(env: Any, body_name: str) -> int:
    ...
    if len(matches) != 1:
        raise RuntimeError(
            f"R012 expected one tensor body named {body_name}; found {matches} in {names}"
        )
    return matches[0]
...
def _tensor_pose_evidence(
    *, raw_world_position: Sequence[float], quaternion: Sequence[float], env_origin: Sequence[float]
    ...
    if raw.shape != (3,) or origin.shape != (3,) or not np.all(np.isfinite(np.concatenate((raw, origin, q)))):
        raise RuntimeError("R012 tensor pose is malformed")
    return {
...
def _resolve_pinch_scene_geometry(env: Any) -> dict[str, Any]:
    ...
    if left_body == right_body:
        raise RuntimeError("R012 inner-finger body resolution is not unique")
...
def _run_geometry_attachment_preflight(
    ...
    *,
    ...
    """Validate live tensor geometry without reading or synchronizing USD world state."""

    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R012 tensor sanity did not run after exact 75-step reset")
    inventory = geometry["inventory"]
    robot = env.scene["robot"].data
    body_names = [str(name) for name in env.scene["robot"].body_names]
    left_index = int(inventory["left_robot_body_tensor_index"])
    right_index = int(inventory["right_robot_body_tensor_index"])
    index_checks = {
        "left_index_in_range": 0 <= left_index < len(body_names),
        "right_index_in_range": 0 <= right_index < len(body_names),
        "indices_distinct": left_index != right_index,
        "left_name_exact": (
            0 <= left_index < len(body_names)
            and body_names[left_index] == inventory["left_robot_body_tensor_name"]
        ),
        "right_name_exact": (
            0 <= right_index < len(body_names)
            and body_names[right_index] == inventory["right_robot_body_tensor_name"]
        ),
    }
    if not all(index_checks.values()):
        raise RuntimeError("R012 tensor body index/name ownership sanity failed")
    live = _live_pinch_bounds(env, geometry)
    left_center = np.asarray(
        live["left"]["reconstructed_bounds_env_local"]["collision_center_env_local_m"],
        dtype=np.float64,
    )
    right_center = np.asarray(
        live["right"]["reconstructed_bounds_env_local"]["collision_center_env_local_m"],
        dtype=np.float64,
    )
    cube_bounds = live["cube"]["reconstructed_bounds_env_local"]
    cube_dimensions = 2.0 * np.asarray(
        cube_bounds["aabb_half_extents_env_local_m"], dtype=np.float64
    )
    separation = float(np.linalg.norm(right_center - left_center))
    sep_min, sep_max = (
        float(value)
        for value in geometry_attachment_preflight_contract[
            "pad_collision_center_separation_m_inclusive"
        ]
    )
    dim_min, dim_max = (
        float(value)
        for value in geometry_attachment_preflight_contract[
            "cube_aabb_dimension_m_each_inclusive"
        ]
    )
    cube_tensor_position = np.asarray(_host(env.scene["rubiks_cube"].data.root_pos_w[0]), dtype=np.float64)
    cube_tensor_quaternion = _quat_normalize_wxyz(
        _host(env.scene["rubiks_cube"].data.root_quat_w[0])
    )
    joint_tensor = np.asarray(_host(robot.joint_pos[0]), dtype=np.float64)
    reset_cube = fresh_reset["objects"]["rubiks_cube"]
    reset_joint = np.asarray(fresh_reset["robot"]["joint_position_rad"], dtype=np.float64)
    reset_cube_position = np.asarray(reset_cube["position_world_m"], dtype=np.float64)
    reset_cube_quaternion = _quat_normalize_wxyz(reset_cube["quaternion_world_wxyz"])
    cube_position_exact = np.array_equal(cube_tensor_position, reset_cube_position)
    cube_quaternion_exact = bool(
        np.array_equal(cube_tensor_quaternion, reset_cube_quaternion)
        or np.array_equal(cube_tensor_quaternion, -reset_cube_quaternion)
    )
    joint_position_exact = np.array_equal(joint_tensor, reset_joint)
    all_finite = bool(
        np.all(
            np.isfinite(
                np.concatenate(
                    (
                        ...
                        right_center,
                        cube_dimensions,
                        cube_tensor_position,
                        cube_tensor_quaternion,
                        joint_tensor,
                    )
                )
            )
    ...
    geometry_hashes = {
        role: geometry["static_body_local_collision_geometry"][role]["canonical_sha256"]
        for role in ("left", "right", "cube")
    }
    passed = bool(
        all_finite
        and sep_min <= separation <= sep_max
        and bool(np.all(cube_dimensions >= dim_min))
        and bool(np.all(cube_dimensions <= dim_max))
        and cube_position_exact
        and cube_quaternion_exact
        and joint_position_exact
        and fresh_reset.get("passed") is True
        and geometry.get("dynamic_usd_world_bounds_used") is False
    )
    return {
        "schema_version": "vla-wam-shared-v3e006-r012-live-tensor-geometry-sanity-preflight-v1",
        "status": (
            "passed_r012_live_tensor_geometry_sanity_preflight"
            if passed
            else "failed_r012_live_tensor_geometry_sanity_preflight"
        ),
        "passed": passed,
        "model_request_count": 0,
        "behavioral_episode_count": 0,
        "geometry_attachment_preflight_contract": geometry_attachment_preflight_contract,
        "geometry_attachment_preflight_contract_sha256": geometry_attachment_preflight_digest,
        "dynamic_state_source": geometry_attachment_preflight_contract["dynamic_state_source"],
        ...
        "static_body_local_geometry_sha256": geometry_hashes,
        "tensor_body_index_name_ownership_checks": index_checks,
        "live_tensor_geometry": live,
        "pad_collision_center_separation_m": separation,
        "pad_collision_center_separation_m_inclusive": [sep_min, sep_max],
        "cube_aabb_dimensions_m": cube_dimensions.tolist(),
        "cube_aabb_dimension_m_each_inclusive": [dim_min, dim_max],
        "fresh_reset_tensor_identity": {
            "live_cube_position_tensor_world_m": cube_tensor_position.tolist(),
            "fresh_reset_cube_position_world_m": reset_cube_position.tolist(),
            "live_cube_quaternion_world_wxyz": cube_tensor_quaternion.tolist(),
            "fresh_reset_cube_quaternion_world_wxyz": reset_cube_quaternion.tolist(),
            "live_robot_joint_position_rad": joint_tensor.tolist(),
            "fresh_reset_robot_joint_position_rad": reset_joint.tolist(),
            "cube_position_bitwise_equal": cube_position_exact,
            "cube_quaternion_sign_invariant_bitwise_equal": cube_quaternion_exact,
            "robot_joint_position_bitwise_equal": joint_position_exact,
            "passed": cube_position_exact and cube_quaternion_exact and joint_position_exact,
        },
        "all_live_tensor_values_finite": all_finite,
        "scene_env_origin_world_m": _host(env.scene.env_origins[0]),
    }
...
def _write_registered_historical_state(
    ...
    *,
    ...
    if position_key != "base_link_position_world_m" or quaternion_key != "base_link_quaternion_world_wxyz":
        raise RuntimeError("R012 historical state lacks explicit base_link pose semantics")
...
def _run_registered_pose_hold(
    ...
    *,
    ...
    if hold_steps != 30 or required_final_steps != 10:
        raise RuntimeError("R012 pose-hold step contract differs")
...
def _solve_registered_ik(
    ...
    *,
    ...
    if (
        ...
        raise RuntimeError("R012 solver initialization differs from its frozen historical source")
    initialized = _write_registered_historical_state(
        env, source=source, frames=frames, eef_index=eef_index
    )
    ...
    for waypoint in initialization["waypoints"]:
        ...
        if (
            ...
            raise RuntimeError("R012 waypoint schedule differs from the registered finite solver")
...
def _archived_r008_open_contact_materialize_and_gate_never_called(
    ...
    *,
    ...
    """Archived predecessor topology; R012 dispatch cannot call this function."""

    raise RuntimeError("archived R008 object-servo construction is unreachable in R012")
    ...
    if lifecycle_contract != {
        ...
        raise RuntimeError("R012 lifecycle contract differs")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R012 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R012 open-contact target contains a nonfinite value")
    ...
    if termination is None:
        ...
        if captured.shape != (13,) or not np.all(np.isfinite(captured)):
            raise RuntimeError("R012 captured-q handoff is malformed")
        ...
        if episode_before_handoff != [765]:
            raise RuntimeError("R012 handoff did not begin at step 765")
    ...
    if len(trace) != 1290 or _host(env.episode_length_buf) != [1365]:
        raise RuntimeError("R012 construction action/counter total differs")
    if captured_joint_target is None or episode_before_handoff is None or len(servo_rows) != 360:
        raise RuntimeError("R012 object servo or q handoff evidence is incomplete")
    return _finalize_unchanged_gates(
        env,
        obs=obs,
        stage_name=stage_name,
...
def _pinch_geometry_materialize_and_gate(
    ...
    *,
    ...
    if registered_lifecycle != expected_lifecycle:
        raise RuntimeError("R012 construction lifecycle contract differs at materialization")
    if _host(env.episode_length_buf) != [75]:
        raise RuntimeError("R012 candidate actions must begin at exact reset step 75")
    ...
    if not np.all(
        ...
        raise RuntimeError("R012 frozen geometry target contains a nonfinite value")
    ...
    if (
        ...
        raise RuntimeError(
            "R012 rank-stage collision inventory/body-relative geometry differs "
    ...
    if (
        ...
        raise RuntimeError("R012 cube collision-center reconstruction is invalid")
    ...
    def run_cartesian_step(
        *,
        ...
        if bool(terminated[0]) or bool(truncated[0]):
            ...
            raise RuntimeError(
                "R012 construction environment terminated; registered candidate "
    ...
    if episode_before_handoff != [1095]:
        raise RuntimeError("R012 captured-q handoff did not begin at registered step 1095")
    ...
    for step in range(600):
        ...
        if termination is not None:
            raise RuntimeError(
                "R012 construction environment terminated during captured-q settle"
            )
    ...
    if obs is None or len(trace) != 1620 or len(acquisition_trace) != 1020:
        raise RuntimeError("R012 retained action trace length differs")
    if len(contact_trace) != 1620 or _host(env.episode_length_buf) != [1695]:
        raise RuntimeError("R012 contact trace or lifecycle counter differs")
    if len(settled) != 10:
        raise RuntimeError("R012 final scientific gate window differs")
...
def main() -> None:
    ...
    CURRENT_STAGE = "load_hash_bound_r012_contracts"
    ...
    def _open_exact_stage_environment(
        *,
        ...
        env, cfg = create_env(
            ...
            policy="v3e006_r012_zero_model_contact_consistent_state_repair",
    ...
    def _close_stage_environment(env: Any, lifecycle: dict[str, Any], *, retained_failure: BaseException | None) -> None:
        ...
        try:
            ...
            if retained_failure is None:
                raise
            print(
                f"environment close raised after retained R012 failure: "
    ...
    # R012 first validates finite live tensor/index/static-geometry sanity.
    # This environment closes before any diagnostic or candidate environment.
    preflight_env = None
    ...
    try:
        (
            ...
            role="live_physics_tensor_geometry_sanity_preflight",
        )
        CURRENT_STAGE = "r012_geometry_attachment_preflight_resolve_relative_bounds"
        preflight_geometry = _resolve_pinch_scene_geometry(preflight_env)
        CURRENT_STAGE = "r012_geometry_attachment_preflight_live_tensor_sanity"
    ...
    if GEOMETRY_ATTACHMENT_PREFLIGHT is None:
        raise RuntimeError("R012 geometry attachment preflight produced no receipt")
    ...
    if GEOMETRY_ATTACHMENT_PREFLIGHT["passed"] is not True:
        CURRENT_STAGE = "r012_geometry_attachment_preflight_failed_blocking_candidates"
        video_path = (
            args.output_dir
            / "videos"
            / "v3e006_r012_geometry_attachment_preflight_failed.mp4"
        )
        video_path.parent.mkdir(parents=True)
        _write_video(video_path, video_frames)
        report = {
            "schema_version": "vla-wam-shared-v3e006-r012-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R012",
            "status": "r012_geometry_attachment_preflight_failed_candidates_not_evaluated",
            "passed": False,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
            ...
            "r011_predecessor_results": _binding(
            ...
            "release_boundary": (
                "the registered live-tensor sanity preflight failed, so every diagnostic, "
    ...
    for diagnostic in candidate_schedule["known_reachable_diagnostics"]:
        ...
        try:
            ...
            LAST_PARTIAL_STAGES = {
                "r012_live_diagnostics": diagnostics,
        ...
        if not diagnostics[-1]["passed"]:
            CURRENT_STAGE = "r012_known_reachable_diagnostic_failed_blocking_candidates"
            video_path = (
                args.output_dir
                / "videos"
                / "v3e006_r012_known_reachable_diagnostic_failed.mp4"
            )
            video_path.parent.mkdir(parents=True)
            _write_video(video_path, video_frames)
            report = {
                ...
                "study_id": "vla_wam_language_steerability_v3",
                "amendment_id": "V3-E006-R012",
                "status": "r012_known_reachable_diagnostic_failed_candidates_not_evaluated",
                "passed": False,
                "model_request_count": 0,
                "behavioral_episode_count": 0,
                "geometry_attachment_preflight_count": 1,
                "r012_live_diagnostic_count": len(diagnostics),
                "repair_candidate_evaluation_count": 0,
                "state_candidate_count": 0,
                "candidate_budget": 4,
                "diagnostic_budget": 4,
                ...
                "video": _binding(video_path),
                "release_boundary": (
                    "registered diagnostic failure blocks every R012 candidate and all behavior; "
    ...
    if len(diagnostics) != 4 or not all(row["passed"] for row in diagnostics):
        raise RuntimeError("R012 candidate evaluation reached without four passed diagnostics")
    ...
    CURRENT_STAGE = "retain_r012_candidate_search"
    video_path = args.output_dir / "videos" / "v3e006_r012_state_repair_search.mp4"
    video_path.parent.mkdir(parents=True)
    _write_video(video_path, video_frames)
    passed = accepted is not None
    if task_prompt is None:
        raise RuntimeError("R012 candidate search created no exact E004 stage environment")
    report = {
            "schema_version": "vla-wam-shared-v3e006-r012-state-repair-result-v1",
            "study_id": "vla_wam_language_steerability_v3",
            "amendment_id": "V3-E006-R012",
            "status": (
                ...
                if passed
                else "r012_candidate_budget_exhausted_no_valid_state_pair"
            ),
            "passed": passed,
            "model_request_count": 0,
            "behavioral_episode_count": 0,
...
if __name__ == "__main__":
    ...
    if construction_failure is not None:
        print(
            "retained R012 failure; exiting child nonzero before SimulationApp.close to prevent "
        ...
        try:
            ...
            raise RuntimeError(
                f"SimulationApp.close raised after completed R012 search: "
