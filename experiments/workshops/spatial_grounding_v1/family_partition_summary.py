...
def _counts(freeze: Mapping[str, Any], smoke: list[Mapping[str, Any]], results: list[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for family_row in freeze["families"]:
        ...
        families[family] = {
            ...
            "physical_accepted": sum(row["outcome"] == "full_six" and row["accepted"] for row in family_results),
            "physical_rejected": sum(row["outcome"] in {"full_six", "terminal_prefix"} and not row["accepted"] for row in family_results),
            ...
            "by_side": {
                side: {
                    "physical_accepted": sum(row["outcome"] == "full_six" and row["accepted"] and row.get("side") == side for row in family_results),
                    "physical_rejected": sum(row["outcome"] in {"full_six", "terminal_prefix"} and not row["accepted"] and row.get("side") == side for row in family_results),
                }
                for side in sorted({row.get("side") for row in family_results if isinstance(row.get("side"), str)})
            },
        }
    return families
...
def _smoke(
    *, config: Mapping[str, Any], freeze: Mapping[str, Any], campaigns: Mapping[str, Path],
    ...
    for row in entries:
        ...
        if (expected.get((family, index)) != design or not root.is_dir() or not receipt.is_file()
        ...
        records.append({**dict(row), "verification": _record(receipt), "recheck": _record(recheck)})
...
def compile_summary(
    *, config_path: Path, workers_root: Path, output: Path,
    ...
    rank_claims = set()
    for rank in range(4):
        rank_root = workers_root / "workers" / str(rank)
        ...
        if rank_claim.get("rank") != rank or rank_claim.get("root") != str(rank_root.resolve()):
        ...
        for slot in expected:
            ...
            if slot_claim.get("rank") != rank or slot_claim.get("slot") != slot:
            ...
            results.append({
                **slot, "outcome": "terminal_prefix" if isinstance(early, Mapping) else "full_six",
                ...
                "trials": verified.get("trials"), "warmup_binding": _record(slot_root / "reset_warmup")
                if (slot_root / "reset_warmup").is_file() else None,
            })
