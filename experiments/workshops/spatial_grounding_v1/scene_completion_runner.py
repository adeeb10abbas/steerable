"""Measure a completion scene, then run the unchanged six-trial SGW qualification.
...
from .scene_completion_contract import scored_names, measured_goal_supports, measured_side
...
def main():
    ...
    launch={'design':row,'scene':scene,'design_sha256':hashlib.sha256(args.design.read_bytes()).hexdigest(),
            ...
            'source_sha256':{n:hashlib.sha256(Path(__file__).with_name(n).read_bytes()).hexdigest()
                             for n in ('scene_design.py','scene_design_task.py','scene_completion_runner.py','scene_completion_contract.py')},
    ...
    try:
        ...
        if args.capture_only:
            env.close(); return
        offsets={n:objects[n]['geometric_center_offset_root_local_xyz_m'] for n in scored_names(row['family'])}
        ...
        if row['family']=='LAT':
            ...
            meta['goal_supports']=measured_goal_supports(row['family'], objects)
            ...
            meta['geometry_guard_support_ids']=[s['name'] for s in row['supports'] if s['name'] not in scored_names(row['family'])]
        side=measured_side(row['family'],objects)
        if row.get('side') in ('left','right') and side != row['side']:
            raise ValueError('Measured side differs from the registered side')
        meta[{'LAT':'approach_side','HEIGHT':'upper_support_side','DIST':'bowl_side'}[row['family']]]=side
        candidate=FixtureCandidate.from_json({'candidate_id':row['design_id'],'family':row['family'],
            ...
            'task_asset':scene['path'],'object_poses':{n:{'position_m':objects[n]['root_position_env_local_xyz_m'],
                   'quaternion_wxyz':objects[n]['root_quaternion_world_wxyz']} for n in scored_names(row['family'])},'metadata':meta})
