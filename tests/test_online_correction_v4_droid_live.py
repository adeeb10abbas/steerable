...
class DroidReviewFixTests(unittest.TestCase):
    ...
    def test_object_kinematic_state_uses_object_pose_when_grabbed(self) -> None:
        import numpy as np
        ...
        backend = object.__new__(LiveRoboLabBackend)
        backend.config = config
        backend.control_tick = 3
        backend._initial_supported_z = 0.81
        backend._latest_raw_obs = None
        backend.modules = {
            "get_world": lambda _env: SimpleNamespace(
                get_pose=lambda name, env_id=0: (
                    np.asarray([0.50, 0.12, 0.85], dtype=np.float32),
            ...
            scene={
                ...
                "robot": SimpleNamespace(
                    data=SimpleNamespace(
                        ...
                        body_pos_w=np.asarray([[[0.41, 0.10, 0.71]]], dtype=np.float32),
                    )
                ),
            },
        )
