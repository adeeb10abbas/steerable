...
from tools.validate_v3e006_r011 import (
    ValidationError,
    canonical_sha,
    slerp,
    ...
    validate_r011_runtime_aliases,
    validate_preflight_source_topology_text,
    validate_static,
...
ART = ROOT / "artifacts/vla_wam_shared_v3/phase_e/canonical_stage_localization_v3e006_r011"
...
def test_real_r010_closure_and_mutations() -> None:
    value = load(R010 / "results/results.json")
    validate_r010_oracle_failure_closure(value)
    for key, replacement in (
        ...
        with pytest.raises(ValueError):
            validate_r010_oracle_failure_closure(bad)
    bad = deepcopy(value)
    bad["raw_result"]["sha256"] = "0" * 64
    with pytest.raises(ValueError):
        validate_r010_oracle_failure_closure(bad)
    bad = deepcopy(value)
    bad["geometry_oracle_finding"]["quantitative_signature"][
        "failed_collision_prim_count"
    ] = 4
    with pytest.raises(ValueError):
        validate_r010_oracle_failure_closure(bad)


def test_registration_is_finite_prospective_and_preserves_r010_schedule() -> None:
    registration = load(ART / "repair_registration.json")
    schedule = load(ART / "gates/candidate_schedule.json")
    predecessor = load(R010 / "gates/candidate_schedule.json")
    assert registration["predecessor_repair_amendment_id"] == "V3-E006-R010"
    ...
    for new_key in (
        ...
        with pytest.raises(ValidationError):
            validate_r011_runtime_aliases(bad, predecessor["candidate_pairs"])
...
def test_pinch_contract_and_mutations() -> None:
    ...
    for key, value in (
        ...
        ("expected_configured_physics_scene_path", "/wrongScene"),
        ("physics_to_usd_call_count", 2),
        ("simulation_forward_before_tensor_snapshot", True),
        (
...
def _preflight_fixture() -> tuple[dict, dict]:
    ...
    fresh["passed"] = True
    preflight = {
        "schema_version": "vla-wam-shared-v3e006-r011-geometry-attachment-preflight-v1",
        "status": "passed_r011_scene_specific_relative_bound_tensor_world_oracle_preflight",
        "passed": True,
        ...
        "physics_to_usd_synchronization_call_arguments": {
            ...
            "physics_scene_path_integer": 4609,
            "update_to_usd": True,
        ...
        "stage_and_scene_identity": {
            ...
            "stage_cache_id_before": 918273645,
            "stage_cache_id_after": 918273645,
            "stage_cache_id_valid_nonzero_and_unchanged": True,
            "root_layer_identifier_before": "anon:fixture-stage",
            "root_layer_identifier_after": "anon:fixture-stage",
            "root_layer_identifier_unchanged": True,
            "configured_physics_scene_path": "/physicsScene",
            "resolved_physics_scene_paths": ["/physicsScene"],
            "unique_configured_physics_scene": True,
            "physics_scene_path_integer": 4609,
...
def test_geometry_attachment_preflight_recomputation_and_mutations() -> None:
    ...
    for mutate in (
        lambda value: value.__setitem__(
            "physics_to_usd_synchronization_call", "wrong_call"
        ),
        lambda value: value.__setitem__(
            "physics_to_usd_synchronization_call_count", 2
        ),
        lambda value: value.__setitem__(
            "physics_or_action_steps_between_tensor_snapshot_and_oracle", 1
        ),
        lambda value: value.__setitem__(
            "simulation_forward_before_tensor_snapshot", True
        ),
        lambda value: value.__setitem__(
            "physics_to_usd_synchronized_reset_steps", 74
        ),
        lambda value: value.__setitem__(
            "physics_to_usd_setting_path", "/wrong"
        ),
        lambda value: value.__setitem__(
            "physics_to_usd_setting_after_one_shot_sync", True
        ),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "stage_cache_id_after", 918273646
        ),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "resolved_physics_scene_paths", ["/wrongScene"]
        ),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "resolved_physics_scene_paths", ["/physicsScene", "/secondScene"]
        ),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "physics_scene_path_integer", 0
        ),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "physics_scene_path_integer", 1122334456
        ),
        lambda value: value[
            "physics_to_usd_synchronization_call_arguments"
        ].__setitem__("physics_scene_path_integer", 1122334456),
        lambda value: value["stage_and_scene_identity"].__setitem__(
            "root_layer_identifier_after", "anon:other-stage"
        ),
    ):
        bad = deepcopy(preflight)
        mutate(bad)
...
def test_scene_sync_source_ast_order_and_forbidden_calls() -> None:
    rows = {
        "path_int": "    scene_path_int = int(PhysicsSchemaTools.sdfPathToInt(configured_scene_path))",
        "snapshot": "    live = _live_pinch_bounds(env, geometry)",
        "before": "    setting_before = settings.get_as_bool(setting_path)",
        "sync": "    update_transformations_scene(scene_path_int, True, False)",
        "after": "    setting_after = settings.get_as_bool(setting_path)",
        "bbox": "    bbox_cache = UsdGeom.BBoxCache()",
        "xform": "    xform_cache = UsdGeom.XformCache()",
    }

    def fixture(order: tuple[str, ...], extra: str | None = None) -> str:
        body = [rows[key] for key in order]
        if extra is not None:
            body.insert(1, f"    {extra}")
        body.append("    return live")
        return "def _run_geometry_attachment_preflight(env, geometry):\n" + "\n".join(body) + "\n"
    ...
    assert validate_preflight_source_topology_text(fixture(registered_order)) is True
    runtime_source = (
        ROOT
        / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r011/state_repair_gate.py"
    ).read_text(encoding="utf-8")
    assert validate_preflight_source_topology_text(runtime_source) is True
    for order in (
        ("path_int", "sync", "snapshot", "before", "after", "bbox", "xform"),
        ("path_int", "snapshot", "before", "bbox", "sync", "after", "xform"),
        ("path_int", "snapshot", "before", "sync", "bbox", "after", "xform"),
    ):
        with pytest.raises(ValidationError):
            validate_preflight_source_topology_text(fixture(order))
    for forbidden in (
        "env.step(action)",
        "env.sim.forward()",
        "env.sim.render()",
        "update_transformations(False, True, False, False)",
        "settings.set_bool(setting_path, True)",
    ):
        with pytest.raises(ValidationError):
            validate_preflight_source_topology_text(
                fixture(registered_order, forbidden)
            )
    wrong_arguments = fixture(registered_order).replace(
        "update_transformations_scene(scene_path_int, True, False)",
        "update_transformations_scene(scene_path_int, True, True)",
    )
    with pytest.raises(ValidationError):
        validate_preflight_source_topology_text(wrong_arguments)
    wrong_path_dataflow = fixture(registered_order).replace(
        "PhysicsSchemaTools.sdfPathToInt(configured_scene_path)",
        "PhysicsSchemaTools.sdfPathToInt('/physicsScene')",
    )
    with pytest.raises(ValidationError):
        validate_preflight_source_topology_text(wrong_path_dataflow)
...
def test_scientific_gate_ast_identity_and_materializer_prohibitions() -> None:
    old = ROOT / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r010/state_repair_gate.py"
    new = ROOT / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r011/state_repair_gate.py"
    for name in (
        "_contact_forces",
        "_contact_coverage",
        "_reference_bounds",
    ...
    assert "update_transformations_scene" in oracle
    assert "PhysicsSchemaTools" in oracle
    assert "sdfPathToInt" in oracle
    assert "SimulationContext" in oracle
    assert "StageCache" in oracle
    assert "env.step" not in oracle
    source = new.read_text(encoding="utf-8")
    assert "R010_RESULTS_SHA256" in source
    assert (
        R010_RESULTS_SHA256
        == "d4762cd8f4db539e760a79ce9e36f81d49455cb76836d8df5dadf902e6b78869"
    )
...
def test_static_package_and_outer_contract() -> None:
    ...
    outer = (ROOT / "tools/run_v3e006_r011_state_repair.py").read_text(encoding="utf-8")
    assert 'REMOTE_BRANCH = "experiment/v3e006-r011-scene-sync-repair"' in outer
    assert "passed_r011_state_repair_not_released_for_behavior" in outer
    assert "r011_candidate_budget_exhausted_no_valid_state_pair" in outer
    assert "r011_geometry_attachment_preflight_failed_candidates_not_evaluated" in outer
