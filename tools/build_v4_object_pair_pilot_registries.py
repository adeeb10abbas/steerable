#!/usr/bin/env python3
"""Build disjoint C7 engineering-pilot reset and policy-seed registries."""
...
from tools.build_v4_object_pair_reset_registry import (
    OBJECT_SPECS,
...
DEFAULT_SEED_OUTPUT = (
    ROOT
    / "artifacts/online_correction_v4/setup/"
    "object_pair_pilot_seed_registry.candidate.json"
)
...
def pilot_env_seeds(seed_manifest: dict[str, Any]) -> tuple[int, ...]:
    ...
    if (
        len(seeds) != EXPECTED_PILOT_COUNT
        or len(set(seeds)) != EXPECTED_PILOT_COUNT
        or seeds != tuple(range(2110000800, 2110000824))
    ):
        raise ValueError("seed manifest does not bind the expected 24 C7 pilot resets")
    return seeds
    ...
    *,
    ...
    for offset, env_seed in enumerate(env_seeds):
        ...
        resets[str(env_seed)] = {
            ...
            "positions_robot_base_m": {
                name: [
                    float(spec["base_position_robot_m"][0]) + dx,
                    float(spec["base_position_robot_m"][1]) + dy,
                    float(spec["base_position_robot_m"][2]),
                ]
    ...
    reset_registry = {
        **base_reset,
        ...
        "release_boundary": (
            "Model-blind C7 engineering-pilot reset candidate only. These 24 "
            "resets are disjoint from confirmatory C7 resets and require G2/G3 "
    ...
    policy_seed_registry = {
        "schema_version": "v4-object-pair-engineering-pilot-seed-registry-v1",
        ...
        "source": source,
        "release_boundary": (
            "Prospective C7 engineering-pilot allocation only. G5-G8 control "
