#!/usr/bin/env python3
"""Build the frozen C8 engineering-pilot manifest and pilot-only runtime lock."""
...
from experiments.online_correction_v4.second_stack import direct_prompt  # noqa: E402
        ...
        rows.append(
            {
                ...
                "runtime_bound": True,
                "episode_id": f"online_correction_v4-C8-pilot-{offset:02d}-{identity}",
                ...
                "factors": factors,
                "prefix_group_id": f"C8-pilot-independent-{offset:02d}",
                "execution_group": (
                ...
                "prompt_recipe": {
                    "template": direct_prompt(goal),
                    "object_role": "green block",
                    "reference_role": "yellow block",
                },
            }
        )
...
def build_groot_seed_registry(
    *,
    ...
    return {
        ...
        "release_boundary": (
            "Allows only the 24 prospectively allocated, excluded C8 engineering-"
    ...
    *,
    ...
    return {
        **candidate,
        ...
        "release_boundary": (
            "Released only for the 24 excluded C8 engineering-pilot episodes. "
            "These resets are disjoint from confirmatory C8 resets."
        ),
    }
...
def receipt_binding(
    *,
    ...
    return {
        "passed": passed,
        "family_ids": family_ids if family_ids is not None else ["C8"],
...
def build_runtime_lock(
    *,
    ...
    if not runtime_image_digest.startswith("sha256:"):
        raise ValueError("runtime_image_digest must begin with sha256:")
    ...
    blocked = {
        family: (
            "Not included in the fixture-scoped C8 engineering-pilot release"
        )
        for family in ("C1", "C2", "C3", "C4", "C5", "C6", "C7")
    }
    receipts = {
    ...
    return {
        ...
        "writer_contract": {
            ...
            "output_parent_uri": "/data/users/ali/vla_wam/raw/v4/g7-second-stack",
        ...
        "policies": {
            POLICY_ID: {
                ...
                "integration_commit": GR00T_COMMIT,
...
def main() -> None:
    ...
    parser.add_argument(
        "--runtime-root",
        default="/data/users/ali/vla_wam/src/steerable-v4-c8-g7",
    )
    parser.add_argument(
    ...
    parser.add_argument("--groot-seed-registry-out", type=Path, required=True)
        ...
        canonical_json_bytes(
            {
                ...
                "release_boundary": (
                    "Frozen C8 engineering-pilot queue only; these rows are excluded "
                    "from the 768 confirmatory episodes."
                ),
            }
        ),
    )
    groot_registry = build_groot_seed_registry(
        rows=rows,
        source_path=args.pilot_seed_registry.resolve(),
    )
    write_exclusive(
        args.groot_seed_registry_out.resolve(),
        canonical_json_bytes(groot_registry),
    )
    ...
    runtime_lock = build_runtime_lock(
        ...
        groot_seed_registry_path=args.groot_seed_registry_out.resolve(),
        ...
        json.dumps(
            {
                ...
                "groot_seed_registry": artifact(args.groot_seed_registry_out.resolve()),
