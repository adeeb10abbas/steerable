#!/usr/bin/env python3
"""Hash-close the validated V3-E006-R008 zero-model state-search exhaustion."""
...
TERMINAL = "r008_candidate_budget_exhausted_no_valid_state_pair"
...
def main() -> None:
    ...
    require(not output.exists(), f"refusing to overwrite R008 closure: {output}")
    ...
    raw = load(child_path)
    require(raw.get("status") == TERMINAL and raw.get("passed") is False, "R008 terminal status differs")
    require(
        raw.get("accepted_candidate_rank") is None
        and raw.get("accepted_states") is None,
        "R008 accepted a state",
    )
    require(
        raw.get("model_request_count") == raw.get("behavioral_episode_count") == 0,
        "R008 is not zero-model",
    )
    diagnostics = raw.get("known_reachable_diagnostics")
    attempts = raw.get("attempts")
    require(isinstance(diagnostics, list) and len(diagnostics) == 4, "R008 diagnostics differ")
    require(all(row.get("passed") is True for row in diagnostics), "R008 diagnostics did not all pass")
    require(isinstance(attempts, list) and len(attempts) == 4, "R008 candidate count differs")
    require([row.get("candidate_rank") for row in attempts] == [1, 2, 3, 4], "R008 rank order differs")
    require(all(row.get("passed") is False for row in attempts), "R008 exhaustion contains a passing pair")
    ...
    memo_path.write_text(
        ...
        f"{counts['frame_identity_pass_count']}/8. Normal gripper contact and intended "
        "cube-gripper contact passed 0/8.\n\n"
        "R008 made zero model requests and zero behavioral episodes. Behavioral activation remains "
        "blocked. The frozen validator initially rejected exact quaternion antipodes introduced by "
        "the frozen runtime's sign canonicalizer. An additive post-execution amendment applied that "
        "exact canonicalizer to expected actions and delegated every remaining check to the frozen "
        "validator; the untouched raw result then passed target revalidation. The zero-byte failed "
        "receipt and authoritative receipt are both retained.\n",
        encoding="utf-8",
    )
    manifest = {
        "schema_version": "vla-wam-shared-v3e006-r008-closure-manifest-v1",
        ...
        "closure_validator": binding(
            ROOT / "tools/validate_v3e006_r008_closure.py", repo_relative=True
        ),
