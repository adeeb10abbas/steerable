...
def exact_mcnemar(b,c):
    n=b+c
    if not n:return {"discordant_total":0,"p_two_sided":1.0}
    p=min(1.0,2*sum(math.comb(n,i) for i in range(min(b,c)+1))/(2**n))
    return {"discordant_total":n,"p_two_sided":p}
def main():
    ...
    b=sum(l["success"] and not r["success"] for l,r in zip(left,right)); c=sum((not l["success"]) and r["success"] for l,r in zip(left,right))
    gap_boot=boot(gap,mean)
    result={"schema_version":"vla-wam-shared-v3e003-results-v1","registered_cells":len(reg["queue"]),"valid_behavioral_episodes":len(rows),"source_files":sources,"directions":{"left":{"n":len(left),"successes":sum(v["success"] for v in left),"wilson_95":wilson(sum(v["success"] for v in left),len(left)),"failure_taxonomy":counts(left)},"right":{"n":len(right),"successes":sum(v["success"] for v in right),"wilson_95":wilson(sum(v["success"] for v in right),len(right)),"failure_taxonomy":counts(right)}},"paired":{"n":len(pairs),"success_gap_right_minus_left":{"mean":gap_boot,"values":gap},"mcnemar":{"left_success_right_failure":b,"left_failure_right_success":c,**exact_mcnemar(b,c)},"depth_contrast_right_minus_left_m":{"mean":boot(depth,mean),"median":boot(depth,median),"sign_test":sign_test(depth),"values_m":depth},"endpoint_shift_right_minus_left_m":{"mean":boot(endpoint,mean),"median":boot(endpoint,median),"values_m":endpoint},"equivalence":{"binary_gap":{"margin":reg["equivalence_margins"]["binary_gap_abs"],"bootstrap_ci":gap_boot,"ci_excludes_margin":gap_boot["low"]>reg["equivalence_margins"]["binary_gap_abs"] or gap_boot["high"]<-reg["equivalence_margins"]["binary_gap_abs"]},"depth_contrast":{"margin_m":reg["equivalence_margins"]["requested_side_depth_contrast_abs_m"],"bootstrap_ci":boot(depth,mean),"ci_within_margin":boot(depth,mean)["low"]>-reg["equivalence_margins"]["requested_side_depth_contrast_abs_m"] and boot(depth,mean)["high"]<reg["equivalence_margins"]["requested_side_depth_contrast_abs_m"]}},"success_gap_values":gap},"symmetry_residual":{"max_m":max((v.get("symmetry_residual",{}).get("max_m",float("nan")) for v in rows),default=None),"all_within_1mm":all(v.get("symmetry_residual",{}).get("max_m",1)>-1 and v.get("symmetry_residual",{}).get("max_m",1)<=.001 for v in rows)},"equivalence_margins":reg["equivalence_margins"],"status":"complete" if len(rows)==54 and len(left)==27 and len(right)==27 else "incomplete"}
