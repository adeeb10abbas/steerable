#!/usr/bin/env python3
"""Render one immutable simulator-only Kubernetes Job per horizontal G3 path seed."""
...
class G3RenderError(ValueError):
    """Raised when a model-blind G3 bundle is not immutable and complete."""


def require(condition: bool, message: str) -> None:
    if not condition:
        raise G3RenderError(message)
...
def _read_spec(path: Path) -> tuple[dict[str, Any], str, str]:
    ...
    try:
        ...
        raise G3RenderError(f"invalid G3 render spec JSON: {path}") from exc
...
def render(spec_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    for index, seed in enumerate(seeds):
        ...
        seed_output = _seed_output_parent(
    ...
    require(
        len(set(seed_output_parents)) == len(seeds),
        "G3 seed output parents must be unique",
    )
    ...
    manifest = {
        ...
        "release_boundary": (
            "Creating this bundle runs only zero-inference G3 path-wave checks at "
