...
def render(spec_path: Path, output_root: Path) -> dict[str, Any]:
    ...
    stem = f"v4-g3-{fixture_stem}-scr-{attempt}-{spec_sha256[:10]}"
    worst_job_name = f"{stem}-j{max(len(scripted_jobs) - 1, 0):03d}"
    lane.assert_k8s_metadata_name(
        worst_job_name,
        context="G3 scripted job",
    )
