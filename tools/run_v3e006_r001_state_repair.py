#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R001 state repair."""
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R001 attempt: {output_root}")
    ...
    remote_head = subprocess.check_output(
        ["git", "-C", str(study_root), "ls-remote", "origin", "refs/heads/experiment/v3e006-r001-state-repair"],
        text=True,
    ).strip().split()
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r001.state_repair_gate",
    ...
    result = {
        ...
        "status": "completed_r001_candidate_search" if passed_process else "infrastructure_invalid_r001_state_repair",
