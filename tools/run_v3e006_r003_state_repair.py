#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R003 state repair."""
...
def child_process_completed(returncode: int, report_path: Path | None, payload: Any) -> bool:
    """Accept only an exact normal R003 terminal report, never a failure report."""
    ...
    if payload.get("model_request_count") != 0 or payload.get("behavioral_episode_count") != 0:
        return False
    diagnostic_count = payload.get("r003_live_diagnostic_count")
    ...
    if payload.get("status") == "r003_known_reachable_diagnostic_failed_candidates_not_evaluated":
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R003 attempt: {output_root}")
    ...
    remote_head = subprocess.check_output(
        ["git", "-C", str(study_root), "ls-remote", "origin", "refs/heads/experiment/v3e006-r003-state-repair"],
        text=True,
    ).strip().split()
    ...
    if source_gate.get("status") != "passed_before_first_r003_live_diagnostic_candidate_or_model_request":
        parser.error("R003 source-push gate did not pass prospectively")
    if (
        source_gate.get("model_request_count") != 0
        or source_gate.get("behavioral_episode_count") != 0
        or source_gate.get("r003_live_diagnostic_count") != 0
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r003.state_repair_gate",
    ...
    launch = {
        "schema_version": "vla-wam-shared-v3e006-r003-state-repair-launch-v1",
        ...
        "r003_live_diagnostic_count_before_launch": 0,
        "r003_live_candidate_evaluation_count_before_launch": 0,
        ...
        "remote_branch_equality": {"remote": "origin", "ref": "refs/heads/experiment/v3e006-r003-state-repair", "commit": remote_head[0]},
    ...
    result = {
        "schema_version": "vla-wam-shared-v3e006-r003-state-repair-harness-result-v1",
        "created_at_utc": utcnow(),
        "status": "completed_r003_candidate_search" if passed_process else "infrastructure_invalid_r003_state_repair",
        ...
        "r003_live_diagnostic_count": int(child_payload.get("r003_live_diagnostic_count", 0)) if child_payload else 0,
