#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R006 state repair."""
...
def child_process_completed(returncode: int, report_path: Path | None, payload: Any) -> bool:
    """Accept only an exact normal R006 terminal report, never a failure report."""
    ...
    if payload.get("model_request_count") != 0 or payload.get("behavioral_episode_count") != 0:
        return False
    diagnostic_count = payload.get("r006_live_diagnostic_count")
    ...
    if payload.get("status") == "r006_known_reachable_diagnostic_failed_candidates_not_evaluated":
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R006 attempt: {output_root}")
    ...
    remote_head = subprocess.check_output(
        ["git", "-C", str(study_root), "ls-remote", "origin", "refs/heads/experiment/v3e006-r006-state-repair"],
        text=True,
    ).strip().split()
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r006.state_repair_gate",
    ...
    launch = {
        "schema_version": "vla-wam-shared-v3e006-r006-state-repair-launch-v1",
        ...
        "r006_live_diagnostic_count_before_launch": 0,
        "r006_live_candidate_evaluation_count_before_launch": 0,
        ...
        "remote_branch_equality": {"remote": "origin", "ref": "refs/heads/experiment/v3e006-r006-state-repair", "commit": remote_head[0]},
    ...
    result = {
        "schema_version": "vla-wam-shared-v3e006-r006-state-repair-harness-result-v1",
        "created_at_utc": utcnow(),
        "status": "completed_r006_candidate_search" if passed_process else "infrastructure_invalid_r006_state_repair",
        ...
        "r006_live_diagnostic_count": int(child_payload.get("r006_live_diagnostic_count", 0)) if child_payload else 0,
