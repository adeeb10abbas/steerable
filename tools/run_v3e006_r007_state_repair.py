#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R007 state repair."""
...
def child_process_completed(returncode: int, report_path: Path | None, payload: Any) -> bool:
    """Accept only an exact normal R007 terminal report, never a failure report."""
    ...
    if payload.get("model_request_count") != 0 or payload.get("behavioral_episode_count") != 0:
        return False
    diagnostic_count = payload.get("r007_live_diagnostic_count")
    ...
    if payload.get("status") == "r007_known_reachable_diagnostic_failed_candidates_not_evaluated":
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R007 attempt: {output_root}")
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r007.state_repair_gate",
    ...
    launch = {
        "schema_version": "vla-wam-shared-v3e006-r007-state-repair-launch-v1",
        ...
        "r007_live_diagnostic_count_before_launch": 0,
        "r007_live_candidate_evaluation_count_before_launch": 0,
    ...
    result = {
        "schema_version": "vla-wam-shared-v3e006-r007-state-repair-harness-result-v1",
        "created_at_utc": utcnow(),
        "status": "completed_r007_candidate_search" if passed_process else "infrastructure_invalid_r007_state_repair",
        ...
        "r007_live_diagnostic_count": int(child_payload.get("r007_live_diagnostic_count", 0)) if child_payload else 0,
