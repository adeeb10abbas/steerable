#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R008 state repair."""
...
REMOTE_BRANCH = "experiment/v3e006-r008-object-servo-repair"
COMPLETED_CHILD_STATUSES = {
    "passed_r008_state_repair_not_released_for_behavior": True,
    "r008_candidate_budget_exhausted_no_valid_state_pair": False,
    "r008_known_reachable_diagnostic_failed_candidates_not_evaluated": False,
}
...
def child_process_completed(returncode: int, report_path: Path | None, payload: Any) -> bool:
    """Accept only an exact normal R008 terminal report, never a failure report."""
    ...
    if payload.get("model_request_count") != 0 or payload.get("behavioral_episode_count") != 0:
        return False
    diagnostic_count = payload.get("r008_live_diagnostic_count")
    ...
    if payload.get("status") == "r008_known_reachable_diagnostic_failed_candidates_not_evaluated":
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R008 attempt: {output_root}")
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r008.state_repair_gate",
    ...
    launch = {
        "schema_version": "vla-wam-shared-v3e006-r008-state-repair-launch-v1",
        ...
        "r008_live_diagnostic_count_before_launch": 0,
        "r008_live_candidate_evaluation_count_before_launch": 0,
    ...
    result = {
        "schema_version": "vla-wam-shared-v3e006-r008-state-repair-harness-result-v1",
        "created_at_utc": utcnow(),
        "status": "completed_r008_candidate_search" if passed_process else "infrastructure_invalid_r008_state_repair",
        ...
        "r008_live_diagnostic_count": int(child_payload.get("r008_live_diagnostic_count", 0)) if child_payload else 0,
