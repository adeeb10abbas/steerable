#!/usr/bin/env python3
"""Failure-retaining outer launcher for the zero-request R011 state repair."""
...
REMOTE_BRANCH = "experiment/v3e006-r011-scene-sync-repair"
COMPLETED_CHILD_STATUSES = {
    "passed_r011_state_repair_not_released_for_behavior": True,
    "r011_candidate_budget_exhausted_no_valid_state_pair": False,
    "r011_known_reachable_diagnostic_failed_candidates_not_evaluated": False,
    "r011_geometry_attachment_preflight_failed_candidates_not_evaluated": False,
}
...
def child_process_completed(returncode: int, report_path: Path | None, payload: Any) -> bool:
    """Accept only an exact normal R011 terminal report, never a failure report."""
    ...
    if payload.get("model_request_count") != 0 or payload.get("behavioral_episode_count") != 0:
        return False
    diagnostic_count = payload.get("r011_live_diagnostic_count")
    ...
    if payload.get("status") == "r011_geometry_attachment_preflight_failed_candidates_not_evaluated":
        return (
            payload.get("geometry_attachment_preflight_count") == 1
            and diagnostic_count == 0
            and count == 0
        )
    if payload.get("status") == "r011_known_reachable_diagnostic_failed_candidates_not_evaluated":
...
def main() -> None:
    ...
    if output_root.exists():
        parser.error(f"refusing to overwrite R011 attempt: {output_root}")
    ...
    child_argv = [
        str(args.python), "-m", "experiments.v3.phase_e.canonical_stage_localization_v3e006_r011.state_repair_gate",
    ...
    launch = {
        "schema_version": "vla-wam-shared-v3e006-r011-state-repair-launch-v1",
        ...
        "r011_live_diagnostic_count_before_launch": 0,
        "r011_live_candidate_evaluation_count_before_launch": 0,
    ...
    result = {
        "schema_version": "vla-wam-shared-v3e006-r011-state-repair-harness-result-v1",
        "created_at_utc": utcnow(),
        "status": "completed_r011_candidate_search" if passed_process else "infrastructure_invalid_r011_state_repair",
        ...
        "geometry_attachment_preflight_count": int(child_payload.get("geometry_attachment_preflight_count", 0)) if child_payload else 0,
        "r011_live_diagnostic_count": int(child_payload.get("r011_live_diagnostic_count", 0)) if child_payload else 0,
