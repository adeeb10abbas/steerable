"""Shared Isaac natural-grasp live positive/negative control for all fixtures."""
...
FIXTURE_DEFAULTS: dict[str, dict[str, Any]] = {
    "horizontal": {
        ...
        "goal": "left",
        "receipt_schema": "v4-horizontal-natural-grasp-live-positive-control-v1",
        "infra_schema": "v4-horizontal-natural-grasp-live-positive-control-infra-v1",
        "geometry_kind": "horizontal",
    },
    "object_pair": {
        "environment_seed": 2100040000,
        "goal": "left",
        "receipt_schema": "v4-object-pair-natural-grasp-live-positive-control-v1",
        "infra_schema": "v4-object-pair-natural-grasp-live-positive-control-infra-v1",
        "geometry_kind": "planar",
    },
    "reference_binding": {
        "environment_seed": 2100010000,
        "goal": "left",
        "receipt_schema": "v4-reference-binding-natural-grasp-live-positive-control-v1",
        "infra_schema": "v4-reference-binding-natural-grasp-live-positive-control-infra-v1",
        "geometry_kind": "planar",
    },
    "vertical": {
        ...
        "infra_schema": "v4-vertical-natural-grasp-live-positive-control-infra-v1",
        "geometry_kind": "vertical",
    },
    "containment": {
        "environment_seed": 2100030000,
        "goal": "inside",
        "receipt_schema": "v4-containment-natural-grasp-live-positive-control-v1",
        "infra_schema": "v4-containment-natural-grasp-live-positive-control-infra-v1",
        "geometry_kind": "containment",
    },
}
...
def _resolve_geometry(
    *,
    ...
    margin = float(geometry_contract["support_edge_margin_m"])
    if geometry_kind == "horizontal":
    ...
    if geometry_kind in {"vertical", "containment"}:
        fixture_geometry = _fixture_geometry_from_registry(registry_payload, fixture_id)
    return geometry_from_scene_for_fixture(
...
def _parser(fixture_id: str) -> argparse.ArgumentParser:
    defaults = FIXTURE_DEFAULTS[fixture_id]
    parser = argparse.ArgumentParser(
        description=f"Live Isaac natural-grasp control for {fixture_id}"
    )
    ...
    parser.add_argument("--environment-seed", type=int, default=defaults["environment_seed"])
    parser.add_argument("--scale", type=float, default=0.5)
    parser.add_argument("--goal", default=defaults["goal"])
...
def main(*, fixture_id: str, argv: list[str] | None = None) -> int:
    if fixture_id not in FIXTURE_DEFAULTS:
        raise ValueError(f"unsupported fixture_id: {fixture_id}")
    defaults = FIXTURE_DEFAULTS[fixture_id]
    args = _parser(fixture_id).parse_args(argv)
    ...
    try:
        ...
        geometry = _resolve_geometry(
            fixture_id=fixture_id,
            geometry_kind=str(defaults["geometry_kind"]),
            task_frame_dict=task_frame_dict,
        ...
        receipt = {
            ...
            "scripted_controller_config": controller_config,
        ...
        _write_json_exclusive(output_dir / "scripted_trajectory.json", trajectory_result)
        ...
        infra = {
            "schema_version": defaults["infra_schema"],
            "attempt_id": args.attempt_id,
            "fixture_id": fixture_id,
