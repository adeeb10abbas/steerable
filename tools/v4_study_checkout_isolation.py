    ...
    *,
    ...
    if apply:
        provision_script = (
            ...
            f'  git -C "$TARGET" fetch origin {BRANCH} >/dev/null 2>&1; '
            f'elif [ -d "$TEMPLATE/.git" ]; then '
            f'  git clone --local "$TEMPLATE" "$TARGET" >/dev/null 2>&1; '
            ...
            f'fi; '
            f'git -C "$TARGET" checkout "$PIN" >/dev/null 2>&1; '
            f'git -C "$TARGET" reset --hard "$PIN" >/dev/null 2>&1; '
            f'git -C "$TARGET" clean -fd >/dev/null 2>&1; '
