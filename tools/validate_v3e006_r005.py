#!/usr/bin/env python3
"""Fail-closed validator for prospective and raw V3-E006-R005 evidence."""
...
def validate_scientific_selection(report: Mapping[str, Any], harness: Mapping[str, Any]) -> None:
    ...
    require(report.get("r005_live_diagnostic_count") == len(diagnostics), "child diagnostic count differs")
    require(harness.get("r005_live_diagnostic_count") == len(diagnostics), "harness diagnostic count differs")
    attempts = report.get("attempts")
    if report.get("status") == "r005_known_reachable_diagnostic_failed_candidates_not_evaluated":
    ...
    require(harness.get("status") == "completed_r005_candidate_search", "harness status differs")
...
def validate_static(root: Path, *, require_source_gate: bool = True) -> dict[str, Any]:
    ...
    require(registration.get("predecessor_repair_amendment_id") == "V3-E006-R004", "predecessor ID differs")
    ...
    predecessor = registration["r004_predecessor"]
    require(predecessor.get("closure_commit") == R004_CLOSURE_COMMIT, "R004 closure commit differs")
    require(predecessor["results"].get("sha256") == R004_RESULTS_SHA256, "R004 results digest differs")
    require(predecessor["raw_result"].get("sha256") == R004_RAW_RESULT_SHA256, "R004 raw digest differs")
    predecessor_results_path = resolve_bound(root, predecessor["results"], "R004 predecessor results")
    try:
        validate_r004_timeout_closure(load(predecessor_results_path))
    except ValueError as exc:
        raise ValidationError(str(exc)) from exc

    require(schedule.get("repair_amendment_id") == "V3-E006-R005", "schedule ID differs")
    require(schedule.get("status") == "frozen_before_any_r005_live_diagnostic_candidate_or_model_request", "schedule status differs")
    ...
    require(schedule.get("r004_predecessor") == predecessor, "schedule predecessor differs")
    ...
    require(registration.get("construction_horizon_contract") == horizon, "registration horizon differs")
    require(registration.get("construction_horizon_contract_sha256") == horizon_sha, "registration horizon digest differs")
    ...
    r004_schedule_path = resolve_bound(root, schedule["r004_target_correction_schedule"], "R004 target/correction schedule")
    r004_schedule = load(r004_schedule_path)
    require(schedule["known_reachable_diagnostics"] == r004_schedule["known_reachable_diagnostics"], "R004 diagnostics changed")
    require(schedule["candidate_pairs"] == r004_schedule["candidate_pairs"], "R004 candidate targets/order changed")
    require(correction == r004_schedule["residual_correction_contract"], "R004 residual correction changed")
    require(
        schedule.get("residual_correction_contract_sha256")
        == r004_schedule.get("residual_correction_contract_sha256"),
        "R004 residual-correction digest changed",
    )
    ...
    for name in (
        ...
        require(function_ast(r004_source, name) == function_ast(r005_source, name), f"unchanged helper differs: {name}")
    source_text = r005_source.read_text(encoding="utf-8")
    ...
    return {
        ...
        "r004_predecessor_results": predecessor["results"],
...
def validate_candidate_root(root: Path, candidate_root: Path) -> dict[str, Any]:
    ...
    if harness.get("process_completed") is True:
        ...
        for key in (
            ...
            "original_v3e006_closure_binding", "r004_predecessor_results", "ood_freeze",
        ...
        require(report.get("status") == "infrastructure_invalid_r005_state_repair", "invalid status differs")
