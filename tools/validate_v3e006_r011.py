#!/usr/bin/env python3
"""Static and target-raw validator for V3-E006-R011."""
...
ART = Path("artifacts/vla_wam_shared_v3/phase_e/canonical_stage_localization_v3e006_r011")
...
def validate_preflight_source_topology_text(source: str) -> bool:
    ...
    require(function is not None, "R011 scene-sync preflight function absent")
    ...
    require(
        ...
        "R011 tensor/sync/setting/cache call multiplicity differs",
    )
    require(
        len(scene_integer_assignments) == 1,
        "R011 scene path integer assignment multiplicity differs",
    )
    ...
    require(
        ...
        "R011 scene path-to-integer dataflow differs",
    )
    ...
    require(
        ...
        "R011 scene-specific synchronization arguments differ",
    )
    ...
    require(
        ...
        "R011 tensor snapshot / scene sync / post-setting / fresh-cache order differs",
    )
    forbidden_motion = {
        name
    ...
    require(not forbidden_motion, "R011 preflight contains a step/forward/render call")
    require(
        not any(name.rsplit(".", 1)[-1] == "update_transformations" for name, _ in named),
        "R011 preflight contains the global PhysX synchronization call",
    )
    require(
        not any(
        ...
        "R011 preflight mutates or otherwise accesses the setting outside exact reads",
    )
    return True
...
def validate_candidate_state(
    ...
    """Independently regenerate R011 geometry commands and unchanged selection."""
    ...
    require(
        ...
        == 0,
        "R011 prohibited state write or branch differs",
    )
    require(
        ...
        "R011 unconditional execution or handoff differs",
    )
...
def _archived_validate_static_r008(root: Path, *, source_gate_required: bool) -> dict[str, Any]:
    ...
    require(registration.get("repair_amendment_id") == "V3-E006-R011", "registration ID differs")
    ...
    r008_source = root / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r011/state_repair_gate.py"
    for name in (
    ...
    require("object_space_servo_command" in material and "_normal_joint_equilibrium_step" in material, "R011 controller topology absent")
    gate_summary = None
...
def validate_r011_runtime_aliases(
    ...
    require(
        ...
        "R011 changed R010 candidate pairs or runtime-driving aliases",
    )
...
def validate_static(root: Path, *, source_gate_required: bool) -> dict[str, Any]:
    ...
    require(
        ...
        == "prospectively_registered_before_any_r011_live_preflight_diagnostic_candidate_or_model_request"
    ...
    require(
        ...
        and schedule.get("r011_geometry_attachment_preflight_count") == 0
        and schedule.get("r011_live_diagnostic_count") == 0
        and schedule.get("r011_live_candidate_evaluation_count") == 0
    ...
    validate_r011_runtime_aliases(schedule["candidate_pairs"], r010_schedule["candidate_pairs"])
    require(
        ...
        "R011 selection/geometry algorithm identifier differs",
    )
    require(
        schedule.get("scene_sync_source_bindings")
        == registration.get("scene_sync_source_bindings")
        == registration.get("frozen_inputs", {}).get("r011_scene_sync_source_bindings")
        == SCENE_SYNC_SOURCES,
    ...
    archived = schedule.get("archived_predecessor_contracts", {})
    require(
        archived.get("status") == "archived_lineage_only_not_active_r011_runtime_evidence"
    ...
    r011_source = root / "experiments/v3/phase_e/canonical_stage_localization_v3e006_r011/state_repair_gate.py"
    for name in (
        "_contact_forces",
        "_contact_coverage",
        "_reference_bounds",
        ...
        require(function_ast(r010_source, name) == function_ast(r011_source, name), f"scientific helper differs: {name}")
    for name in (
        ...
        require(
            ...
            == function_ast(r011_source, name).replace("R011", "R010"),
    ...
    for filename, names in pure_function_inventory.items():
        ...
        new_module = r011_source.parent / filename
        for name in names:
            require(
                function_ast(old_module, name)
                == function_ast(new_module, name).replace("R011", "R010"),
                f"R010 pure controller math changed: {filename}:{name}",
            )
    source = r011_source.read_text(encoding="utf-8")
    require("requests.post" not in source and "policy_server" not in source, "model endpoint exists")
    ...
    validate_preflight_source_topology(r011_source)
    preflight_ast = function_ast(r011_source, "_run_geometry_attachment_preflight")
    require(
        ...
        "R011 scene-specific preflight topology differs",
    )
    require(
        function_call_count(
            r011_source,
    ...
    main_ast = function_ast(r011_source, "main")
    require(
        ...
        "R011 dispatch reaches an unregistered topology",
    )
    gate_summary = None
    if source_gate_required:
        gate_path = artifact / "source_push_gate.json"
...
def validate_terminal_diagnostic_pattern(
    ...
    failed = (
        report.get("status")
        == "r011_known_reachable_diagnostic_failed_candidates_not_evaluated"
    )
    require(isinstance(diagnostics, list), "diagnostic rows absent")
    require(
        report.get("r011_live_diagnostic_count") == len(diagnostics)
...
def validate_terminal_geometry_preflight_pattern(
    ...
    failed = (
        report.get("status")
        == "r011_geometry_attachment_preflight_failed_candidates_not_evaluated"
    )
    if failed:
        require(
            ...
            and report.get("r011_live_diagnostic_count") == 0
            and report.get("attempts") == []
            and report.get("repair_candidate_evaluation_count") == 0
            and report.get("state_candidate_count") == 0
            and report.get("accepted_candidate_rank") is None
...
def validate_candidate_root(root: Path, candidate_root: Path) -> dict[str, Any]:
    ...
    require(
        harness.get("status") == "completed_r011_candidate_search"
    ...
    if preflight_failure:
        ...
        require(
            report.get("status")
            == "r011_candidate_budget_exhausted_no_valid_state_pair",
    ...
    for key in (
        ...
        "original_v3e006_closure_binding", "r010_predecessor_results", "ood_freeze",
    ...
    for key, row in execution.get("input_bindings", {}).items():
        if key == "scene_sync_sources":
            require(isinstance(row, Mapping), "scene-sync execution sources absent")
            for source_name, source_row in row.items():
                verify(
                    Path(source_row["path"]),
                    source_row,
                    f"execution scene-sync source {source_name}",
                )
        else:
    ...
    require(
        ...
        and report["r010_predecessor_results"]
