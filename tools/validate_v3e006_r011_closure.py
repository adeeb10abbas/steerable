#!/usr/bin/env python3
"""Validate R011's final fail-closed, pre-action scene-sync closure."""
...
from tools.close_v3e006_r011 import TERMINAL, summarize_preflight


ROOT = Path(__file__).resolve().parents[1]
CLOSURE = ROOT / "artifacts/vla_wam_shared_v3/phase_e/canonical_stage_localization_v3e006_r011/results"
...
def validate_compact_contract(results: Mapping[str, Any]) -> None:
    require(
        ...
        "R011 closure terminal/counts differ",
    )
    require(
        ...
        and results.get("intended_r011_construction_scientifically_exhausted")
        is False
        and results.get("final_state_construction_blocker") is True,
        "R011 closure interpretation/release differs",
    )
...
def main() -> None:
    ...
    require(
        manifest.get("status") == TERMINAL
        and manifest.get("model_request_count")
        == manifest.get("behavioral_episode_count")
        == 0
        and manifest.get("behavioral_release_permitted") is False
        and manifest.get("final_state_construction_blocker") is True,
        "manifest terminal/counts/release differ",
    )
    for key, label in (
    ...
    if args.verify_raw:
        ...
        schedule_path = verify(root, results["candidate_schedule"], "result schedule")
        ...
        require(
            ...
            and raw.get("model_request_count")
            == raw.get("behavioral_episode_count")
            == 0,
            "raw pre-action counts differ",
        )
        require(
            raw.get("geometry_attachment_preflight") == preflight,
            "raw preflight copy differs",
        )
        require(
            summarize_preflight(preflight, schedule)
            == results.get("geometry_oracle_finding"),
            "scene-sync geometry finding does not regenerate",
        )
