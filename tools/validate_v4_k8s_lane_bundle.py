...
def _validate_launch_json(data: dict[str, str]) -> dict[str, dict[str, Any]]:
    ...
    for role, document in documents.items():
        ...
        _require(isinstance(document.get("gpu_product"), str) and document["gpu_product"], f"{role} gpu_product missing")
    ...
    _require(service_identity.get("v4-lane-role") == "policy", "simulator policy_wait does not bind policy role")
    _validate_qualification_scope(documents)
    return documents


def _contains_v4_runner(value: str) -> bool:
    lowered = value.lower()
...
def _validate_qualification_scope(documents: dict[str, dict[str, Any]]) -> None:
    ...
    sim_bindings = {
        str(_mapping(raw, "simulator binding")["path"])
    ...
    _require(len(sim_argv) >= 2, "behavioral simulator experiment_argv must be nonempty")
    ...
    _require(argv_runner, "behavioral simulator argv must invoke online_correction_v4 runner")
    _require(binding_runner, "behavioral simulator file_bindings must include online_correction_v4 runner")
    _require(set(argv_runner) & set(binding_runner), "behavioral simulator argv/file_bindings must agree on runner path")


def _label_identity(raw: Mapping[str, Any], label: str) -> dict[str, str]:
    metadata = _mapping(_mapping(raw, label).get("metadata"), f"{label} metadata")
    labels = _mapping(metadata.get("labels"), f"{label} labels")
    identity = {key: str(labels[key]) for key in REQUIRED_IDENTITY_LABELS if key in labels}
    _require(len(identity) == len(REQUIRED_IDENTITY_LABELS), f"{label} lacks unique lane/attempt/config labels")
    return identity
...
def _validate_unique_lane_identity(
    *,
    ...
    policy_pod_labels: Mapping[str, str],
    ...
    for key, value in expected.items():
        _require(policy_pod_labels.get(key) == value, f"policy pod label {key} differs from immutable identity")
        _require(service_selector.get(key) == value, f"policy Service selector {key} differs from immutable identity")
    _require(
        all(key in str(policy_job.get("metadata", {}).get("name", "")) for key in (expected["v4-lane-id"], expected["v4-attempt-id"])),
...
def _validate_policy_port_isolation(
    ...
    *,
    ...
    spec = _mapping(policy_job.get("spec"), "policy Job spec")
    template = _mapping(spec.get("template"), "policy template")
    pod_spec = _mapping(template.get("spec"), "policy pod spec")
    _require(pod_spec.get("hostNetwork") is not True, "policy pod must not share the host network namespace")
    _require(pod_spec.get("shareProcessNamespace") is not True, "policy pod must isolate process namespace")
    containers = _list(pod_spec.get("containers"), "policy containers")
    _require(len(containers) == 1, "policy pod must run exactly one policy process container")
    sidecars = _list(pod_spec.get("initContainers") or [], "policy initContainers")
    _require(not sidecars, "policy pod must not add sidecars that could bind the policy port")
    env = _env_map(_mapping(containers[0], "policy container"), "policy")
    _require("POLICY_PORT" in env, "policy container must bind POLICY_PORT")
    bound_port = int(str(env["POLICY_PORT"].get("value")))
    launch_port = int(str(launch_document.get("policy_port")))
    _require(bound_port == launch_port == service_port, "policy port differs across Job, launch JSON, and Service")
    _require(
        "reserve_policy_port" in bundle_source and "o_excl" in bundle_source.lower(),
        "policy port isolation requires O_EXCL reserve_policy_port preflight in the lane scripts",
    )


def _validate_readiness_binding_verification(
    ...
    *,
    ...
    command = _list(readiness_exec.get("command"), "policy readinessProbe command")
    command_text = " ".join(str(part) for part in command)
    _require("check_policy_ready.py" in command_text, "policy readiness probe must use check_policy_ready.py")
    _require("--checkpoint-loaded" in command_text, "policy readiness probe must require checkpoint-loaded gate")
    _require("--launch-config" in command_text, "policy readiness probe must bind the mounted launch config path")
    _require(POLICY_LAUNCH_CONFIG_MOUNT in command_text, "policy readiness probe must reference the mounted launch config file")
    ready_script = script_root / "check_policy_ready.py"
    _require(ready_script.is_file(), "check_policy_ready.py is missing")
    ready_source = ready_script.read_text(encoding="utf-8")
    _require("launch_config" in ready_source and "sha256" in ready_source, "readiness script lacks launch-config digest verification")
    _require(
        "report.get(\"launch_config\", {}).get(\"sha256\")" in ready_source
        or 'report.get("launch_config", {}).get("sha256")' in ready_source,
        "readiness script must verify post-start launch-config binding against preflight evidence",
    )
    if "--port" in command_text:
        port_flags = [index for index, item in enumerate(command[:-1]) if item == "--port"]
        _require(len(port_flags) == 1, "policy readiness probe must bind one explicit port")
        _require(str(command[port_flags[0] + 1]) == str(launch_document.get("policy_port")), "readiness probe port differs from launch JSON")


def _validate_create_only_contract(root: Path) -> None:
    readme = root / "README.md"
    if readme.is_file():
        text = readme.read_text(encoding="utf-8").lower()
        _require("create" in text and "kubectl apply" in text, "README must document create-not-apply semantics")
        _require("do not" in text or "not use" in text, "README must forbid kubectl apply reuse")
    renderer = REPO_ROOT / "tools/render_v4_k8s_lane_bundle.py"
    _require(renderer.is_file(), "renderer must fail closed on overwrite")
    _require("refusing to overwrite" in renderer.read_text(encoding="utf-8"), "renderer must fail closed on overwrite")
...
def _validate_configmap(configmap: dict[str, Any]) -> tuple[str, dict[str, str], dict[str, dict[str, Any]]]:
    ...
    _require(isinstance(data.get("kube.context"), str) and data["kube.context"], "ConfigMap must bind kube.context")
...
def _validate_job(
    ...
    *,
    ...
    expected_gpu = str(launch_document["gpu_product"])
    ...
    _require(pod_labels.get("v4-lane-role") == role, f"{role} pod lacks exact role label")
    job_labels = _mapping(_mapping(job.get("metadata"), f"{role} metadata").get("labels"), f"{role} Job labels")
    _require(job_labels.get("v4-lane-role") == role, f"{role} Job lacks exact role label")
    ...
    for name in ("LANE_ID", "ATTEMPT_ID", "KUBE_CONTEXT"):
    ...
    if role == "policy":
        ...
        _require(
            "--launch-config" in readiness_text,
            "policy readiness probe must bind the mounted launch config path",
        )
...
def _validate_service(service: dict[str, Any], policy_job: dict[str, Any]) -> dict[str, Any]:
    ...
    _require(selector.get("v4-lane-role") == "policy", "policy Service selector lacks exact policy role")
...
def _validate_generated_from_spec(root: Path, spec_path: Path | None = None) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="v4-lane-render-") as temporary:
...
def validate(
    ...
    *,
    ...
    _validate_create_only_contract(root)
    ...
    lane_identity = _validate_unique_lane_identity(
        ...
        service=service,
        policy_pod_labels=jobs["policy"]["pod_labels"],
        service_selector=service_summary["selector"],
    )
    _validate_policy_port_isolation(
        policy,
        launch_document=launch_documents["policy"],
        service_port=int(service_summary["port"]),
        bundle_source=source,
    )
    pod_spec = _mapping(
        _mapping(_mapping(policy.get("spec"), "policy spec").get("template"), "policy template").get("spec"),
        "policy pod spec",
    )
    policy_container = _mapping(_list(pod_spec.get("containers"), "policy containers")[0], "policy container")
    _validate_readiness_binding_verification(
        policy_container,
        launch_document=launch_documents["policy"],
        script_root=root / "scripts",
    )
...
def main() -> None:
    ...
    try:
        ...
        raise SystemExit(f"V4 Kubernetes lane-bundle validation failed: {exc}") from exc
