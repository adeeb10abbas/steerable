...
COSMOS_GIT_TREE = "1e77852f2240d6e00312342ab29164745042b1ae"
...
def validate_n3_qualification_payload(n3: Mapping[str, Any]) -> None:
    """Validate the released raw qualification schema, not its compact summary."""
    ...
    source = n3.get("source")
    require(isinstance(source, Mapping), "n3_source_identity_missing")
    require(source.get("commit") == COSMOS_COMMIT, "n3_source_identity_changed")
    require(source.get("git_tree") == COSMOS_GIT_TREE, "n3_source_tree_changed")
    checkpoint = n3.get("checkpoint")
    require(isinstance(checkpoint, Mapping), "n3_checkpoint_identity_missing")
    require(checkpoint.get("revision") == CHECKPOINT_REVISION, "n3_checkpoint_revision_changed")
    require(
        checkpoint.get("payload_aggregate_sha256") == CHECKPOINT_AGGREGATE_SHA256,
        "n3_checkpoint_hash_changed",
    )
...
def validate_prerequisites(
    *,
    ...
    validate_n3_qualification_payload(n3)
...
def build_server_command(
    *,
    ...
    require(0 <= start_cell_index < len(CELL_IDS), "invalid_start_cell_index")
    script = Path(source_root) / "workshops/corl2026_world_models/experiments/forecast_layout/n3_behavioral_pilot_job.py"
    return [
...
_MUTABLE_TEMPORAL_STATE_MARKERS = (
    "episode",
    "session",
    "past_key",
    "kv_cache",
...
def _temporal_field_inventory(value: Any, *, component: str) -> dict[str, Any]:
    ...
    for name, field_value in sorted(vars(value).items()):
        ...
        if name in _STATIC_FRAMEWORK_FIELDS:
            ...
            is_runtime_state = any(
                marker in name.lower() for marker in _MUTABLE_TEMPORAL_STATE_MARKERS
            )
            if size == 0:
                classification = "empty_mutable_container"
            elif is_runtime_state:
                classification = "unresolved_mutable_temporal_container"
                unresolved.append(f"{component}.{name}")
            else:
                classification = "nonempty_scanned_configuration_container"
    ...
    try:
        ...
        ready = {
            ...
            "expected_behavioral_request_count":
                (len(CELL_IDS) - args.start_cell_index) * REQUEST_COUNT,
            "planned_block_behavioral_request_count": len(CELL_IDS) * REQUEST_COUNT,
    ...
    try:
        ...
        class BoundN3Client(Cosmos3Client):
            ...
            def begin_episode(self) -> Mapping[str, Any]:
                ...
                require(
                    reset.get("unresolved_mutable_temporal_fields") == [],
                    "server_temporal_state_unresolved",
                )
...
def supervised_logged_child(
    ...
    *,
    ...
    """Launch a child and unconditionally reap its process group and logs."""
    ...
    try:
        ...
        try:
            ...
            _close_process_logs(stdout, stderr)
...
def validate_passed_cell_receipt(
    ...
    require(
        ...
        and COMMIT_RE.fullmatch(source_pins["study_commit"]) is not None,
        "resume_cell_study_commit_invalid",
    )
    require(
        source_pins.get("robolab_commit") == ROBOLAB_COMMIT
        and source_pins.get("cosmos_commit") == COSMOS_COMMIT,
    ...
    require(isinstance(begin, Mapping) and begin.get("passed") is True, "resume_begin_receipt_invalid")
    context_id = begin.get("server_context_id")
    require(isinstance(context_id, str) and context_id, "resume_context_id_missing")
    ...
    require(
        ...
        and end.get("server_request_count") == REQUEST_COUNT
        and end.get("client_request_count") == REQUEST_COUNT
        and end.get("actions_executed") == ACTION_CAP,
...
def discover_completed_prefix(
    ...
    """Return the sole hash-verified contiguous P00 prefix, refusing ambiguity."""
    ...
    for aggregate_path in sorted(
        raw_root.glob("*/publish/n3_behavioral_pilot_receipt.json")
    ):
        attempt_root = aggregate_path.parents[1].resolve()
        aggregate = load_json(aggregate_path, "resume_aggregate_receipt_unreadable")
        require(aggregate.get("schema_version") == QUEUE_RECEIPT_SCHEMA, "resume_aggregate_schema_changed")
        require(aggregate.get("block_id") == BLOCK_ID, "resume_aggregate_block_mismatch")
        require(aggregate.get("model_config") == MODEL_CONFIG, "resume_aggregate_model_mismatch")
        descriptors = aggregate.get("cell_receipts")
        require(isinstance(descriptors, list), "resume_aggregate_descriptors_invalid")
        ...
        require(
            [valid_paths[path] for path in descriptor_paths]
            == list(range(len(descriptor_paths))),
            "resume_aggregate_cell_order_contradiction",
            str(attempt_root),
        )
        if aggregate.get("status") == "passed":
            require(len(descriptor_paths) == len(CELL_IDS), "resume_passed_aggregate_incomplete")
    ...
    context_ids = [receipt["server_begin_receipt"]["server_context_id"] for receipt in receipts]
    require(len(context_ids) == len(set(context_ids)), "resume_server_context_id_reused")
    provenance = {
        "schema_version": "wmf-n3-behavioral-resume-v1",
        "status": "passed",
        "block_id": BLOCK_ID,
...
def _receipt_counts(
    *,
    ...
    return {
        ...
        "unrun_behavioral_cells": planned - start_cell_index - launched,
...
def run_queue(args: argparse.Namespace) -> int:
    ...
    with lock_path.open("a+b") as lock_handle:
        ...
        try:
            ...
            require(
                start_cell_index < len(CELL_IDS),
                "valid_n3_p00_block_already_exists",
            ...
            with installed_signal_handlers():
                try:
                    ...
                    for condition_index in range(start_cell_index, len(CONDITIONS)):
                        ...
                        ) as (child, _child_stdout, _child_stderr):
                            launched += 1
                            immutable_json(
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.mode == "server":
        require(0 <= args.start_cell_index < len(CELL_IDS), "invalid_start_cell_index")
